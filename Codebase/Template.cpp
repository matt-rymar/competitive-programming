...
std::vector<std::vector<int>> readGraph(const int vertices, const int edges,
                                        const bool directed) {
...
std::vector<std::vector<std::pair<int, Weight>>> readGraphWeighted(
    const int vertices, const int edges, const bool directed) {
...
// Note: if two elements are equivalent, sortedTracedCopy preserves the original
// order of the items. (In other words, the sort performed is stable)
template <typename T, typename Comp = std::less<T>>
std::vector<std::pair<int, T>> sortedTracedCopy(const std::vector<T>& vec,
  ...
  std::stable_sort(cpy.begin(), cpy.end(), [&](const auto& p1, const auto& p2) {
    return comp(p1.second, p2.second);
  });  // TODO: change it to normal sort with tiebreaking using the p1.firsts?
  return cpy;
}
...
// std::mt19937 rng(dev());
// auto dist6 = std::uniform_int_distribution<std::mt19937::result_type>(1, 6);
//  auto rand = dist6(rng);
