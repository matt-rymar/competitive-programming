...
// Function for the annoying Google-style output format
...
i64 vFunc(i64 a, i64 s, i64 p)
{
	i64 res = 0;

	for (auto i = naiveExp(a, s) - naiveExp(a % p, s);
		(i > 0) && ((i % p) == 0); i /= p)
		++res;
...
// Note: UpdateData also must have a default constructor. However, the default-initialized value 
//  does not matter and can be arbitrary
// For an example use, see 
//  -- Codeforces 380C--Sereja and Brackets (basic use, only queries and no updates)
//  -- Codeforces 474F--Ant Colony (basic use, slightly more complex accumulation)
//  -- Codeforces 1371F--Raging Thunder (only range queries and range updates, but an elaborate structure to keep track of)
...
//  -- Codeforces 1491C--Pekora and Trampoline (simple range add/point query problem)
...
public:
	...
	// Same as query except only those parts of the range get accumulated, for which the unary predicate pred(Data) == 1
	...
	//	if pred(Data) == 0, then the query analyzes the subsegments (starting with the left half) to see if one of those is valid (or if we are a leaf, it gets ignored)
	// Note that pred can also be a boolean predicate, in which case the early exit feature simply won't do anything
	// Note that the running time now depends on the pred and may be little-omega(lgn)
...
private:
	...
	public:
		...
		// TODO: make this simply call query_if with a predicate which always returns true? (It might make the constant factor of query() worse)
		Data query(const int lo, const int hi)
		{
			if (isDisjoint(lo, hi))
				return Data();
		...
		{
			// We only consider sequences which begin somewhere in our interval (see the standard case below to understand why checking for this is convenient)
	...
	private:
		// The ordering of members is probably cache-unfriendly, but w/e, it's not like pointer-based Segment Tree will be a speed demon
...
struct Data
{
	i64 p = 0; // Kinda wasteful, but w/e
	i64 s = 0;// Kinda wasteful, but w/e
	i64 sum = 0;
	...
	{
		data.sum = vFunc(upd.val, data.s, data.p);
...
std::vector<DefSegmentTree> init(const V<i64>& vec, i64 p)
{
	auto res = std::vector<DefSegmentTree>();
	
	for (int s = 1; s <= 4; ++s)
		res.emplace_back([&](int i) { return Data{ .p = p, .s = s, .sum = vFunc(vec[i], s, p) }; }, vec.size());
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		for (int i = 0; i < q; ++i) {
			...
			if (type == 1) {
				...
				std::cin >> pos >> val;
				for (auto& t : trees)
					t.pointUpdate(pos - 1, UpdateData{ .val = val });
			}
			else {
				i64 s, l, r;
				std::cin >> s >> l >> r;
				res.push_back(trees[s - 1].query(l - 1, r - 1).sum);
			}
		}
