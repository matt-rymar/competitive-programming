...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		auto vec = std::vector<std::pair<int, std::pair<bool, i64>>>();
		for (int i = 0; i < n; ++i) {
			int b, e, h;
			std::cin >> h >> b >> e;
			vec.emplace_back(b, std::make_pair<bool, i64>(false, i64(h)));
			vec.emplace_back(e, std::make_pair<bool, i64>(true, i64(h)));
		}

		std::sort(vec.begin(), vec.end());

		std::multiset<i64> topK;
		std::multiset<i64, std::greater<i64>> rest;
		i64 sum = 0;

		const auto addInt = [&](i64 happiness) {
			if (topK.size() < k) {
				topK.insert(happiness);
				sum += happiness;
			}
			else if (happiness <= (*topK.cbegin())) {
				rest.insert(happiness);
			}
			else {
				auto removeVal = *topK.cbegin();

				topK.erase(topK.begin());
				topK.insert(happiness);
				rest.insert(removeVal);
		...
		const auto removeInt = [&](i64 happiness) {
			if (topK.count(happiness) > 0) {
				topK.erase(topK.find(happiness));
				sum -= happiness;

				if (rest.size() > 0) {
					const auto newVal = *rest.cbegin();
				...
				rest.erase(rest.find(happiness));
			}
		};
