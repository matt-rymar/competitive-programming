...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		const auto isValidPath = [&](const VI& path) -> P<bool, int> {
			std::set<int> visited;
			visited.insert(path[0]);

			auto curPoints = vals[path[0]];
			if (curPoints == k)
				return { true, 0 };

			for (int i = 1; i < path.size(); ++i) {
				if ((curPoints < intervals[path[i]].first) || (intervals[path[i]].second < curPoints))
					return { false, i };
			...
			return { false, path.size() - 1};
		};


		auto curPerm = std::vector<int>(n);
		std::iota(curPerm.begin(), curPerm.end(), 0);

		const auto skipUntil = [&](int skip) {
			const auto pref = std::vector<int>(curPerm.begin(), curPerm.begin() + skip + 1);

			while (true) {
				if (!std::next_permutation(curPerm.begin(), curPerm.end()))
					return false;
		...
		i64 res = 0;
		auto p = isValidPath(curPerm);
		if (p.first)
			++res;
		
		while (skipUntil(p.second)) {
			p = isValidPath(curPerm);
			if (p.first)
				++res;
		}
