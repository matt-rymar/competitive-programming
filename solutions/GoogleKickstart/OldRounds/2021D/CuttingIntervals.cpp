...
std::map<i64, i64> getSizeAmounts(V<P<i64, i64>>& eventPoints, i64 c)
{
	...
	int i = 0;
	for (; (i < eventPoints.size()) && (eventPoints[i].first == eventPoints.front().first); ++i) {
		if (eventPoints[i].second)
			++curIntervals;
		else
			--curIntervals;
	}
	...
	while (i < eventPoints.size()) {
		// Cut points strictly between last two event points
		res[curIntervals] += eventPoints[i].first - eventPoints[i - 1].first - 1;

		int j = i;
		for (; (j < eventPoints.size())
			&& (eventPoints[i].first == eventPoints[j].first)
			&& (!eventPoints[j].second); ++j)
			--curIntervals;
		// Cut points at the event point
		++res[curIntervals];

		for (; (j < eventPoints.size())
			&& (eventPoints[i].first == eventPoints[j].first); ++j)
			++curIntervals;
...
{
	auto sizes = getSizeAmounts(eventPoints, c);

	// Initially, there are eventPoints.size() / 2 intervals
	// Cutting an interval increases this number by one
	i64 res = eventPoints.size() / 2;
	for (auto it = sizes.crbegin(); (it != sizes.crend()) && (c > 0); ++it) {
		auto [noInts, noCuts] = *it;

		noCuts = std::min(c, noCuts);
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		i64 n, c;
		std::cin >> n >> c;
		V<P<i64, i64>> eventPoints;
		for (int i = 0; i < n; ++i) {
			i64 l, r;
			std::cin >> l >> r;
			// Ensure that ends come before beginnings
			eventPoints.emplace_back(l, 1);
			eventPoints.emplace_back(r, 0);
		}

		const auto res = solve(eventPoints, c);
		goog(_testCase, res);
	}
}