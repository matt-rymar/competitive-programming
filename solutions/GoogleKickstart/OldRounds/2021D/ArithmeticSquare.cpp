...
std::optional<int> getD(i64 i, i64 j)
{
	const auto twoD = j - i;
	if (twoD & 1)
		return {};
...
bool isArithmetic(i64 i, i64 j, i64 k)
{
	return ((j - i) == (k - j));
}

int countNonMiddle(const VV<i64>& nums)
{
	int res = 0;
	res += isArithmetic(nums[0][0], nums[1][0], nums[2][0]);
	res += isArithmetic(nums[0][2], nums[1][2], nums[2][2]);
	res += isArithmetic(nums[0][0], nums[0][1], nums[0][2]);
	res += isArithmetic(nums[2][0], nums[2][1], nums[2][2]);
	return res;
}

int solve(const VV<i64>& nums)
{
	std::unordered_map<i64, int> res;
	
	if (auto opt = getD(nums[0][0], nums[2][2]); opt)
		++res[*opt];
	if (auto opt = getD(nums[0][2], nums[2][0]); opt)
		++res[*opt];
	if (auto opt = getD(nums[0][1], nums[2][1]); opt)
		++res[*opt];
	if (auto opt = getD(nums[1][0], nums[1][2]); opt)
		++res[*opt];
	...
	return max + countNonMiddle(nums);
}
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		VV<i64> in;
		in.push_back(readVecSz<i64>(3));
		in.push_back(readVecSz<i64>(2));
		in[1].insert(in[1].begin() + 1, 0);
		in.push_back(readVecSz<i64>(3));

		goog(_testCase, solve(in));
	}
}