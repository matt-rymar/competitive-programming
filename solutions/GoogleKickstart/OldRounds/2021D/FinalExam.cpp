...
std::vector<i64> solve(std::set<i64>& begs, std::set<i64>& ends, const V<i64>& diffs)
{
	std::vector<i64> res;

	const auto removeFromInterval = [&](i64 b, i64 e, i64 i) {
		begs.erase(b);
		ends.erase(e);
	...
	for (const auto d : diffs) {
		auto rightBegIt = begs.lower_bound(d);
		auto leftBegIt = rightBegIt;
		if ((rightBegIt == begs.end()) || (((*rightBegIt) > d) && (rightBegIt != begs.begin())))
			--leftBegIt;
		auto rightEndIt = ends.lower_bound(d);
		auto leftEndIt = rightEndIt;
		if ((rightEndIt == ends.end()) || (((*rightEndIt) > d) && (rightEndIt != ends.begin())))
			--leftEndIt;

		const auto leftBeg = ((*leftBegIt) <= d) ? (*leftBegIt) : NEG_INF<i64>;
		const auto rightBeg = (rightBegIt != begs.end()) ? (*rightBegIt) : POS_INF<i64>;
		const auto leftEnd = ((*leftEndIt) <= d) ? (*leftEndIt) : NEG_INF<i64>;
		const auto rightEnd = (rightEndIt != ends.end()) ? (*rightEndIt) : POS_INF<i64>;

		if ((leftBeg == d) || (rightEnd == d)
			|| ((leftBeg < d) && (leftEnd < leftBeg))) {
			removeFromInterval(leftBeg, rightEnd, d);
			res.push_back(d);
		} else {
			const auto leftDist = (leftEnd > NEG_INF<i64>) ? (d - leftEnd) : POS_INF<i64>;
			const auto rightDist = (rightBeg < POS_INF<i64>) ? (rightBeg - d) : POS_INF<i64>;
			if (leftDist <= rightDist) {
				removeFromInterval(leftBeg, leftEnd, leftEnd);
				res.push_back(leftEnd);
			} else {
				removeFromInterval(rightBeg, rightEnd, rightBeg);
				res.push_back(rightBeg);
			}
		}
	}
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		auto vec = readVecSz<i64>(m);

		const auto res = solve(begs, ends, vec);
		goog(_testCase, res);
	}
}