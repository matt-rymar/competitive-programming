...
public:
	...
	i64 getCountSum(i64 k) const
	{
		return std::accumulate(visitedNodes.cbegin(), visitedNodes.cend(), i64(0),
			[k](auto acc, auto node) {return acc + (node->sizeWithDuplicates / k); });
	}
private:
	static constexpr char baseLetter = 'A';

	struct Node {
		static constexpr size_t lettersInAlphabet = 26;

		std::vector<std::unique_ptr<Node>> children = std::vector<std::unique_ptr<Node>>(lettersInAlphabet);
		bool tick = false;
	...
	Node root;
	std::vector<Node*> visitedNodes; // except for root
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int n, k;
		std::cin >> n >> k;
		auto vec = readVecSz<std::string>(n);

		auto trie = Trie(vec);
		std::cout << "Case #" << i + 1 << ": " << trie.getCountSum(k) << '\n';
	}
}