...
// Note: it should TLE because the radix sort can have O(n^2logn) running time
// However, it seems to give WA, not sure why--pretty sure that sorted order is optimal for the task
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		auto maxLen =
			std::max_element(vec.cbegin(), vec.cend(), [](const auto& s, const auto& t) {return s.size() < t.size(); })->size();
		auto charAt = [](const auto& s, int idx) {if (idx < s.size()) return int(s[idx]); return -1; };
		for (int i = maxLen - 1; i >= 0; --i)
			std::stable_sort(vec.begin(), vec.end(), [&](const auto& s, const auto& t) {return charAt(s, i) < charAt(t, i); });
		i64 score = 0;
		for (int i = 0; i < n; i += k) {
			int lcp = vec[i].size();
			for (int j = i + 1; j < (i + k); ++j) {
				int l = 0;
				while ((l < vec[j].size()) && (l < lcp)) {
					if (vec[j - 1][l] != vec[j][l])
						break;
					++l;
				}
				lcp = l;
