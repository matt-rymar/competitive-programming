...
int testCase(int p, int q, const std::vector<i64>& vec, const std::vector<int>& qs)
{
	Data::MOD = p;
	auto tree = DefSegmentTree([&](int i) { return Data{ .prod = vec[i] }; }, vec.size());
	...
	for (int i = 0; i < q; ++i) {
		...
		const auto [newL, newR] = std::minmax((prevL + x) % vec.size(), (prevR + x) % vec.size());
		prevL = newL;
		prevR = newR;

		x = (tree.query(newL, newR).prod + 1) % p;
	}
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		int n, p, q;
		std::cin >> n >> p >> q;
		const auto vec = readVecSz<i64>(n);
		const auto qs = readVecSz<int>((q / 64) + 2);

		std::cout << testCase(p, q, vec, qs) << '\n';
	}
}