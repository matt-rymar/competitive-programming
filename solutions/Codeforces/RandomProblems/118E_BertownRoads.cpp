...
// Returns all bridges in the graph if one exists
std::vector<std::pair<int, int>> getBridges(const std::vector<std::vector<int>>& adj)
{
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		if (auto b = getBridges(adj); b.size() > 0) {
			std::cout << "0\n";
			continue;
		}
		...
		auto arrival = std::vector<int>(adj.size(), -1);
		int curTime = 0;
		auto dfs = y_combinator([&](auto dfs, int cur, int par) -> void {
			arrival[cur] = curTime++;
			for (auto i : adj[cur]) {
				if (i == par)
					continue;
