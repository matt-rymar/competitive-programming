...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int m, n;
		std::cin >> m >> n;
		auto ms = readVecSz<i64>(m);
		auto ns = readVecSz<i64>(n);

		//std::priority_queue<std::pair<i64, std::pair<int, int>>, std::vector<std::pair<i64, std::pair<int, int>>>, std::greater<>> q;
		std::priority_queue<std::pair<i64, std::pair<int, int>>> q;
		for (int i = 0; i < m; ++i) {
			auto vec = readVec<int>([](auto i) {return i - 1; });
			for (auto j : vec)
				q.emplace(ms[i] + ns[j], std::make_pair(i, m + j));
		}
