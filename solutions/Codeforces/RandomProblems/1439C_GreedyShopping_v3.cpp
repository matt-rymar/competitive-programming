...
struct UpdateData
{
	...
	{
		upd.val = std::max(upd.val, rhs.val);
		return upd;
	}
};
...
// Version that uses the segment tree library and query_if/update_if
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		auto rangeSetMax = [&](const int hi, const i64 val) {
			auto setMaxPred = [&](const Data& data) {
				if (data.min >= val)
					return -1;
				if (data.max <= val)
					return 1;
			...
			tree.update_if(0, hi, UpdateData{ val }, setMaxPred);
		};
		auto shopQuery = [&](int lo, i64 val) {
			auto queryPred = [&](const Data& data) {
				if (data.min > val)
					return -1;
				if ((data.max > val) || (data.sum > val))
					return 0;
				val -= data.sum;
				return 1;
			};
			return tree.query_if(lo, vec.size() - 1, queryPred).rangeLen;
		};
