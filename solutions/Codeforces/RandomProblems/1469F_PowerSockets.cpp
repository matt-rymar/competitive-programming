...
{
public:
	SegmentTree(const int lo, const int hi)
		: root(std::make_unique<Node>(lo, hi))
	{ }

	int firstAbove(int64_t k) { return root->firstAbove(k); }
	void rangeAdd(int lo, int hi, int64_t val) { root->rangeAdd(lo, hi, val); }
	int64_t get(int idx) { return root->get(idx); }
...
private:
	...
	public:
		...
		int firstAbove(int64_t k) 
		{
			...
			push();
			if (getLeft()->val >= k)
				return getLeft()->firstAbove(k);
			return getRight()->firstAbove(k - getLeft()->val);
		}
		...
		int64_t get(int idx)
		{
			if (leftmost == rightmost)
				return val;
			push();
			if (idx <= ((leftmost + rightmost) / 2))
				return getLeft()->get(idx);
			return getRight()->get(idx);
		}

		void rangeAdd(int lo, int hi, int64_t diff)
		{
			if (isDisjoint(lo, hi))
				return;
			if (coversUs(lo, hi)) {
			...
			push();
			getLeft()->rangeAdd(lo, hi, diff);
			getRight()->rangeAdd(lo, hi, diff);
	...
	private:
		void push()
		{
			if (leftmost == rightmost)
				return;
			getLeft()->val += lazyAdd * getLeft()->rangeLen();
			getRight()->val += lazyAdd * getRight()->rangeLen();
			getLeft()->lazyAdd += lazyAdd;
			getRight()->lazyAdd += lazyAdd;
			lazyAdd = 0;
		}
		...
		bool isDisjoint(int lo, int hi) { return (rightmost < lo) || (hi < leftmost); }
		bool coversUs(int lo, int hi) { return (lo <= leftmost) && (rightmost <= hi); }
...
int solve(const std::vector<int>& chains, const int k)
{
	if (k == 1)
		return 0;
	auto depths = SegmentTree(0, POS_INF<int>);
	depths.rangeAdd(0, 0, 1);
	int curDepth = 0;
	int noAtDepth = 1;
	int best = -1;
	for (auto i : chains) {
		depths.rangeAdd(curDepth, curDepth, -1);
		depths.rangeAdd(curDepth + 2, curDepth + 1 +  i / 2, 1);
		depths.rangeAdd(curDepth + 2, curDepth + 1 + i / 2 - !(i & 1), 1);
		auto val = depths.firstAbove(k);
		if ((best == -1) || (val < best))
			best = val;
		if ((--noAtDepth) > 0)
			continue;
		while ((noAtDepth = depths.get(++curDepth)) == 0)
			;
	}
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		while ((vec.size() > 0) && (vec.back() < 3))
			vec.pop_back();

		std::cout << solve(vec, k) << '\n';
	}
}