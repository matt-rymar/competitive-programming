...
std::vector<Count> prefCount(std::string_view sv)
{
	auto cur = Count{};
	auto res = std::vector<Count>{ cur };
	res.reserve(sv.size());
	for (auto c : sv) {
		if (c == '?')
			++cur.qs;
...
std::vector<Count> suffCount(std::string_view sv)
{
	auto cur = Count{};
	auto res = std::vector<Count>{ cur };
	res.reserve(sv.size());
	...
	for (int i = 0; i < t; ++i) {
		...
		if (x < y) {
			...
			std::transform(s.cbegin(), s.cend(), s.begin(),
				[](auto c) {if (c == '0') return '1'; if (c == '1') return '0'; return c; });
		}
		auto prefs = prefCount(s);
		auto suffs = suffCount(s);
		int64_t val = 0;
		for (int i = 0; i < s.size(); ++i) {
			if (s[i] == '1')
				val += x * (prefs[i].zeros + prefs[i].qs);
			else
				val += y * (prefs[i].ones);
		}
		int64_t bestVal = val;
		for (int i = 0; i < s.size(); ++i) {
			if (s[i] != '?')
				continue;
			val -= y * (prefs[i].ones + prefs[i].qs) + x * (suffs[i].ones);
			val += x * (prefs[i].zeros) + y * (suffs[i].zeros + suffs[i].qs);
