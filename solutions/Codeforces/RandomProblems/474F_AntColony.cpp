...
struct Data
{
	...
	{
		const auto gcd = std::gcd(left.gcd, right.gcd);
		Data res{ .gcd = gcd };
		if (left.gcd == gcd)
			res.gcdCount += left.gcdCount;
		if (right.gcd == gcd)
			res.gcdCount += right.gcdCount;
		return res;
	}
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		auto tree = DefSegmentTree([&](auto i) {return Data{ vec[i], 1 }; }, vec.size());
		for (int i = 0; i < q; ++i) {
			int l, r;
			std::cin >> l >> r;
			--l; --r;
			std::cout << (r - l + 1) - tree.query(l, r).gcdCount << '\n';
		}
	}
}