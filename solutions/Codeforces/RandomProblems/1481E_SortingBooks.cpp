...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		auto vec = readVec<int>([](auto i) {return i - 1; });
		if (vec.size() == 1) {
			std::cout << "0\n";
			continue;
		}

		auto leftmost = std::vector<int>(vec.size(), -1);
		auto rightmost = leftmost;
		auto count = std::vector<int>(vec.size(), 0);
		auto maxSuffixCount = std::vector<int>(vec.size(), 0);

		// Right-to-left precomp
		++count[vec.back()];
		rightmost[vec.back()] = vec.size() - 1;
		maxSuffixCount.back() = 1;
		for (int i = vec.size() - 2; i >= 0; --i) {
			if (rightmost[vec[i]] < 0)
				rightmost[vec[i]] = i;
			leftmost[vec[i]] = i;
			++count[vec[i]];
			maxSuffixCount[i] = std::max(maxSuffixCount[i + 1], count[vec[i]]);
		}

		auto dp = std::vector<int>(vec.size(), 0);
		if (count[vec[0]] == 1)
			dp[0] = 1;
		int res = dp[0] + maxSuffixCount[1];
		for (int i = 1; i < vec.size(); ++i) {
			dp[i] = dp[i - 1];
			if (i == rightmost[vec[i]]) {
				const auto before = (leftmost[vec[i]] > 0) ? dp[leftmost[vec[i]] - 1] : 0;
				dp[i] = std::max(dp[i], before + count[vec[i]]);
			}
			const auto after = ((i + 1) < vec.size()) ? maxSuffixCount[i + 1] : 0;
			res = std::max(res, dp[i] + after);
		}
		std::cout << vec.size() - res << '\n';
	}
}