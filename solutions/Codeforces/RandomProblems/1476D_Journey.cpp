...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		auto maxLeft = std::vector<std::vector<int>>(n + 1, std::vector<int>(2, -1));
		maxLeft[0][0] = maxLeft[0][1] = 1;
		for (int i = 1; i <= n; ++i) {
			if (s[i - 1] == 'L') {
				maxLeft[i][0] = 1 + maxLeft[i - 1][1];
				maxLeft[i][1] = 1;
			}
			else {
				maxLeft[i][1] = 1 + maxLeft[i - 1][0];
				maxLeft[i][0] = 1;
			}
		}
		auto maxRight = std::vector<std::vector<int>>(n + 1, std::vector<int>(2, -1));
		maxRight[n][0] = maxRight[n][1] = 1;
		for (int i = n - 1; i >= 0; --i) {
			if (s[i] == 'R') {
				maxRight[i][0] = 1 + maxRight[i + 1][1];
				maxRight[i][1] = 1;
			}
			else {
				maxRight[i][1] = 1 + maxRight[i + 1][0];
				maxRight[i][0] = 1;
			}
		}

		for (int i = 0; i <= n; ++i)
			std::cout << maxLeft[i][0] + maxRight[i][0] - 1 << ' ';
		std::cout << '\n';
	}
}