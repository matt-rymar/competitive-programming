...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		for (int i = 0; i < vec.size(); ++i) {
			const auto curCap = vec[i].first;
			const auto curWater = vec[i].second;
			for (int j = 0; j <= i; ++j) {
				for (int k = 0; k <= totalCap; ++k) {
					if (dp[i][j][k] < 0)
						continue;
					dp[i + 1][j][k] = std::max(dp[i + 1][j][k], dp[i][j][k]);
					if (k + curCap <= totalCap)
						dp[i + 1][j + 1][k + curCap] = std::max(dp[i + 1][j + 1][k + curCap], dp[i][j][k] + curWater);
				}
			}
		}
