...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int n;
		std::cin >> n;
		auto vec = readVec<std::pair<int, int>>([](auto p) {--p.first; --p.second; return p; });
		auto rows = std::vector<int>(n, -1);
		auto cols = rows;
		for (const auto& p : vec) {
			rows[p.first] = p.second;
			cols[p.second] = p.first;
		}
		int res = 0;
		auto done = std::unordered_set<std::pair<int, int>>();
		auto bfs = [&done, &rows, &cols](std::pair<int, int> s) {
			if (s.first == s.second)
				return 0;
			std::queue<std::pair<int, int>> q;
			q.push(s);
			done.insert(s);
			auto freeSpots = std::unordered_set<std::pair<int, int>>();
			int visited = 0;
			while (!q.empty()) {
				const auto cur = q.front();
				q.pop();
				freeSpots.emplace(cur.first, cur.first); freeSpots.emplace(cur.second, cur.second);
				++visited;
				const auto sameCol = std::make_pair(cols[cur.first], cur.first);
				const auto sameRow = std::make_pair(cur.second, rows[cur.second]);
				if ((cols[cur.first] >= 0) && (done.count(sameCol) <= 0)) {
					q.push(sameCol);
					done.insert(sameCol);
				}
				if ((rows[cur.second] >= 0) && (done.count(sameRow) <= 0)) {
					q.push(sameRow);
					done.insert(sameRow);
				}
			}
			if (visited < freeSpots.size())
				return visited;
			return visited + 1;
		};
