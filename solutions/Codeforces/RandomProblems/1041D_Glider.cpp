...
int64_t solve(const std::vector<std::pair<int64_t, int64_t>>& vec, const int64_t h)
{
	int r = 1;
	int64_t res = -1;
	auto hRem = h;
	for (int i = 0; i < vec.size(); ++i) {
		while ((r < vec.size()) && ((vec[r].first - vec[r - 1].second) < hRem)) {
			hRem -= vec[r].first - vec[r - 1].second;
			++r;
		}
		res = std::max(res, (vec[r - 1].second - vec[i].first) + hRem);
		if ((i + 1) == r)
			++r;
		else
			hRem += vec[i + 1].first - vec[i].second;
	}
	return res;
}
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int64_t n, h;
		std::cin >> n >> h;
		auto vec = readVecSz<std::pair<int64_t, int64_t>>(n);
		std::cout << solve(vec, h) << '\n';
	}
}