...
void updateVec(std::vector<i64>& left, const std::vector<i64>& right, const std::vector<P<int, int>>& adj)
{
	std::map<i64, i64> counts;
	for (auto i : right)
		++(counts[i]);
	// For each vertex on the left, count how many times the value on the right is excluded
	std::map<i64, std::map<i64, i64>> coolStuff;
	for (auto [i, j] : adj)
		(coolStuff[right[j]][i])++;

	// We don't care about bad nodes
	counts.erase(-1);
	coolStuff.erase(-1);

	const auto init = (counts.empty()) ? -1 : counts.cbegin()->first;
	auto best = V<i64>(left.size(), init);
	bool excluded = true; // Checks whether some entry of best was changed in the last iteration of the loop
	for (auto it = coolStuff.cbegin(); (it != coolStuff.cend()) && (excluded); ++it) {
		excluded = false;
		const auto curVal = it->first;
		const auto curCount = counts[curVal];
		const auto next = counts.upper_bound(curVal);
		const auto nextVal = (next == counts.end()) ? -1 : (next->first);
		for (auto [i, c] : it->second) {
			// If all the occurences of curVal have been excluded and i relied on them (i.e., is not smaller than curVal), 
			// then we need to increase it
			if ((c >= curCount) && (best[i] == curVal)) {
				best[i] = nextVal;
				excluded = true;
			}
		}
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		auto sizes = readVecSz<int>(4);
		auto costs = std::vector<std::vector<i64>>();
		for (auto s : sizes)
			costs.push_back(readVecSz<i64>(s));
		auto edges = std::vector<std::vector<std::pair<int, int>>>();
		for (int i = 0; i < 3; ++i)
			edges.push_back(readVec<std::pair<int, int>>([](auto p) {--p.first; --p.second; return p; }));

		for (int i = 2; i >= 0; --i)
			updateVec(costs[i], costs[i + 1], edges[i]);

		i64 best = -1;
		for (auto i : costs.front()) {
			if (i < 0)
				continue;
			if ((best < 0) || (i < best))
				best = i;
