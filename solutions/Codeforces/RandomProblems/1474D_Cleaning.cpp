...
int main()
{
	...
	std::cin >> t;
	for (int i = 0; i < t; ++i) {
		auto vec = readVec<i64>();

		auto prefixVal = std::vector<i64>{ 0 };
		for (auto i : vec)
			prefixVal.push_back(i - prefixVal.back());
		std::vector<i64> prefixMins{ POS_INF<i64> };
		std::partial_sum(prefixVal.cbegin(), prefixVal.cend(), std::back_inserter(prefixMins),
			[](auto acc, auto val) {return std::min(acc, val); });

		auto suffixVal = std::vector<i64>{ 0, 0 };
		for (auto it = vec.crbegin(); it != vec.crend(); ++it)
			suffixVal.push_back(*it - suffixVal.back());
		suffixVal.pop_back();
		std::vector<i64> suffixMins{ POS_INF<i64> };
		std::partial_sum(suffixVal.cbegin(), suffixVal.cend(), std::back_inserter(suffixMins),
			[](auto acc, auto val) {return std::min(acc, val); });
		std::reverse(suffixVal.begin(), suffixVal.end());
		std::reverse(suffixMins.begin(), suffixMins.end());

		auto isGood = [&](int i) {
			if ((prefixMins[i] < 0) || (suffixMins[i + 1] < 0))
				return false;
			if (vec[i + 1] < prefixVal[i])
				return false;
			i64 val = vec[i + 1] - prefixVal[i];
			if (vec[i] < val)
				return false;
			val = vec[i] - val;
			if (suffixVal[i + 1] != val)
				return false;
			return true;
		};

		bool good = (prefixMins[vec.size()] >= 0) && (prefixVal[vec.size()] == 0);
