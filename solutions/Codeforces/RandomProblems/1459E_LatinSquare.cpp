...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		auto offs = std::vector<int>(3);
		auto perm = std::vector<int>{ 0, 1, 2 };

		for (auto c : ops) {
			switch (c) {
			case 'R':
				offs[perm[1]] = (offs[perm[1]] + 1) % n;
				break;
			case 'L':
				offs[perm[1]] = (n + offs[perm[1]] - 1) % n;
				break;
			case 'D':
				offs[perm[0]] = (offs[perm[0]] + 1) % n;
				break;
			case 'U':
				offs[perm[0]] = (n + offs[perm[0]] - 1) % n;
				break;
			case 'I':
		...
		auto res = mat;
		const auto rowOff = offs[perm[0]];
		const auto colOff = offs[perm[1]];
		const auto valOff = offs[perm[2]];
		for (int i = 0; i < n; ++i) {
			for (int j = 0; j < n; ++j) {
				auto point = std::vector<int>{ i, j, mat[i][j] };
				res[(point[perm[0]] + rowOff) % n][(point[perm[1]] + colOff) % n] = ((point[perm[2]] + valOff) % n) + 1;
			}
		}
