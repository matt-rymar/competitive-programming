...
int solve(const VI& vec, int k)
{
	if (k == 1)
		return *std::min_element(vec.cbegin(), vec.cend());
	if (k >= 3)
		return *std::max_element(vec.cbegin(), vec.cend());
	// Using RMQ just to try it out even though brute force would be linear instead of the nlogn of RMQ
	auto rmq = RMQ(vec);
	auto res = NEG_INF<int>;
	for (int i = 0; i < (vec.size() - 1); ++i)
		res = std::max(res, std::max(rmq.getMin(0, i), rmq.getMin(i + 1, vec.size() - 1)));
	return res;
}
