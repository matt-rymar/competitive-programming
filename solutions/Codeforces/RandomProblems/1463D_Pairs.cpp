...
std::vector<int> nextAvailable(const std::vector<int>& b)
{
	auto res = std::vector<int>(2 * b.size() + 1, POS_INF<int>);
	int idx = b.size() - 1;
	for (int i = 2 * b.size() - 1; i >= 0; --i) {
		if ((idx >= 0) && (i + 1 == b[idx])) {
			res[i] = res[i + 1];
			--idx;
		}
...
int maxSmaller(const std::vector<int>& b)
{
	auto next = nextAvailable(b);
	int curNum = 0;
	int count = 0;
	for (auto i : b) {
		curNum = std::max(next[curNum], next[i]);
		if (curNum == POS_INF<int>)
			break;
		++count;
	}
	return count;
...
std::vector<int> prevAvailable(const std::vector<int>& b)
{
	auto res = std::vector<int>(2 * b.size() + 2, -1);
	int idx = 0;
	for (int i = 2; i <= (2*b.size() + 1); ++i) {
		if ((idx < b.size()) && (i - 1 == b[idx])) {
			res[i] = res[i - 1];
			++idx;
		}
		else {
...
int maxLarger(const std::vector<int>& b)
{
	auto prev = prevAvailable(b);
	int curNum = 2*b.size() + 1;
	int count = 0;
	for (int i = b.size() - 1; i >= 0; --i) {
		curNum = std::min(prev[curNum], prev[b[i]]);
		if (curNum <= 0)
			break;
		++count;
	}
...
int main()
{
	...
	for (int k = 0; k < t; ++k) {
		...
		auto smaller = maxSmaller(vec);
		auto larger = maxLarger(vec);
		auto intersect = smaller + larger - vec.size();
