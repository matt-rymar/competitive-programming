...
//	(Note that in particular, x is allowed to be <lo or >hi)
// WARNING: binary search is notorious for off-by-one errors and so far I've only tested the code below on one task, so I do not have 100% confidence in its correctness
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		const auto ans = ternarySearch(0, n-1, query) + 1;
		std::cout << "! " << ans << '\n';
	}
}