...
// Returns a list of inverses of k modulo p, for k in [0, upTo), or in [0, p) if upTo < 0, more efficiently than one-by-one
...
// See https://codeforces.com/blog/entry/5457 for an explanation of this black magic
// Also, this code may have a type actually not work; do not trust it
template<typename T>
...
// Computes (n choose k) % mod, given a vector of modular inverses (for example, the one computed by modInverseRange)
template<typename T> 
T nChooseKMod(const T n, const T k, const T mod, const std::vector<T>& modInverses)
{
	T res = 1;
...
const auto modInverses = modInverseRange<int64_t>(MOD, 200'000 + 1);
	...
	for (int i = 0; i < t; ++i) {
		...
		for (auto it = vec.cbegin(); it != vec.cend(); ) {
			...
			while ((greaterThanOrEqualTo >= m) && ((equalTo--) > 0))
				res = (res + nChooseKMod(--greaterThanOrEqualTo, m-1, MOD, modInverses)) % MOD;
			it = end;
		}
		std::cout << res << '\n';
	}
