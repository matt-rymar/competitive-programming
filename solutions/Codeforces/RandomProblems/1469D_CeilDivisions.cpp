...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		std::vector<int64_t> toLeave{ 2 };
		while (toLeave.back() * toLeave.back() <= n)
			toLeave.push_back(toLeave.back() * toLeave.back());

		std::vector<P<int, int>> ops;
		for (int i = 3, idx = 1; i < n; ++i) {
			if ((idx < toLeave.size()) && (i == toLeave[idx])) {
				++idx;
				continue;
			}
		...
		if (n != toLeave.back()) {
			ops.emplace_back(n, toLeave.back());
			ops.emplace_back(n, toLeave.back());
		}

		auto last = toLeave.back();
		for (int idx = toLeave.size() - 2; idx >= 0; --idx) {
			ops.emplace_back(last, toLeave[idx]);
			ops.emplace_back(toLeave[idx], last);
		}
