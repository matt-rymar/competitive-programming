...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		auto target = std::accumulate(vec.cbegin(), vec.cend(), i64(0)) / 2;
		auto sumFn = [&](int i) {int64_t sum = 0; for (; i < vec.size(); i += 2) sum += std::abs(vec[i] - 1); return sum; };
		auto evens = sumFn(0);
		for (int j = (evens <= target) ? 0 : 1; j < vec.size(); j += 2)
			vec[j] = 1;
		std::cout << vec << '\n';
	}
}