...
auto overlap = y_combinator([](auto self, i64 l1, i64 r1, i64 l2, i64 r2) -> i64 {
	if (r2 >= l1)
		return std::max(i64(0), std::min(r1, r2) - std::max(l1, l2) + 1);
	else
		return self(l2, r2, l1, r1);
});
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		const auto vec = readVec<int64_t>();

		i64 res = 0;

		for (int i = 0; i < vec.size(); i += 2) {
			i64 left = 1, right = vec[i];

			for (int j = i + 1; j < vec.size(); j += 2) {
				res += overlap(left, right, 0, vec[j]);

				if (vec[j] > right)
					break;
				right -= vec[j];
				left = std::max(i64(0), left - vec[j]);
