...
int main()
{
	...
	std::cin >> t;
	for (int i = 0; i < t; ++i) {
		auto commands = readVec<std::pair<int64_t, int64_t>>();
		int64_t pos = 0;
		int64_t res = 0;
		for (int i = 0; i < commands.size(); ) {
			const auto delta = std::abs(pos - commands[i].second);
			const int64_t dirMult = (commands[i].second < pos) ? -1 : 1;
			const auto until = commands[i].first + delta;
			if (((i + 1) >= commands.size()) || (commands[i + 1].first >= until))
				++res;
			int j = i + 1;
			auto curPos = pos;
			for (; (j < commands.size()) && (commands[j].first < until); ++j) {
				const auto deltaT = commands[j].first - commands[j - 1].first;
				curPos += deltaT * dirMult;
				auto border = POS_INF<int64_t>;
				if (((j + 1) >= commands.size()) || (commands[j + 1].first >= until))
					border = commands[i].second;
				else
					border = curPos + (commands[j + 1].first - commands[j].first) * dirMult;
				auto left = std::min(curPos, border), right = std::max(curPos, border);
				if ((left <= commands[j].second) && (commands[j].second <= right))
					++res;
			}
			pos = commands[i].second;
			i = j;
		}
		std::cout << res << '\n';
	}
