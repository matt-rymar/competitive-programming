...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		auto sizes = readVecSz<i64>(n);
		auto as = readVecSz<i64>(n, [](auto i) {return i - 1; });
		auto bs = readVecSz<i64>(n, [](auto i) {return i - 1; });

		std::map<int, std::map<int, std::map<int, i64>>> memo;
		auto bestStartingAt = y_combinator([&](auto self, int idx, int entry, int exit) -> i64 {
			if (memo[idx][entry].count(exit) > 0)
				return memo[idx][entry][exit];
			const i64 end = std::abs(entry - exit);
			if ((idx == 0) || (entry == exit)) {
				memo[idx][entry][exit] = end;
				return end;
			}
			const auto upperFirst = self(idx - 1, as[idx], bs[idx]) + 2;
			const auto lowerFirst = self(idx - 1, bs[idx], as[idx]) + 2;
			if (entry < exit)
				memo[idx][entry][exit] = std::max(end, entry + (sizes[idx] - exit - 1) + upperFirst);
			else
				memo[idx][entry][exit] = std::max(end, exit + (sizes[idx] - entry - 1) + lowerFirst);
			return memo[idx][entry][exit];
			});

		i64 res = 0;
		for (int i = 1; i < sizes.size(); ++i)
			res = std::max(res, 2 + (sizes[i] - 1)
				+ std::max(bestStartingAt(i - 1, as[i], bs[i]), bestStartingAt(i - 1, bs[i], as[i])));
		std::cout << res << '\n';
	}
}