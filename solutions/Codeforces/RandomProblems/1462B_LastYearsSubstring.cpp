...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		std::string s;
		std::cin >> n >> s;
		const std::string pattern = "2020";
		int longestPref = 0;
		while ((longestPref < pattern.size()) && (s[longestPref] == pattern[longestPref]))
			++longestPref;
		int longestSuff = pattern.size() - 1;
		while ((longestSuff >= 0) && (s[s.size() - pattern.size() + longestSuff] == pattern[longestSuff]))
			--longestSuff;
		longestSuff = pattern.size() - 1 - longestSuff;
		if (longestPref + longestSuff >= pattern.size())
			std::cout << "YES\n";
		else
			std::cout << "NO\n";
	}
