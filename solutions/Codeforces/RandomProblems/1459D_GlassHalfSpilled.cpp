...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		auto totalCap = std::accumulate(vec.cbegin(), vec.cend(), 0, [](auto acc, auto p) {return acc + p.first; });
		auto totalWater = std::accumulate(vec.cbegin(), vec.cend(), 0, [](auto acc, auto p) {return acc + p.second; });
		auto dp = VV<VI>(vec.size() + 1, VVI(vec.size() + 1, VI(totalCap + 1, -1)));
		dp[0][0][0] = 0;
		for (int i = 1; i <= vec.size(); ++i) {
			dp[i][0][0] = 0;
			const auto curCap = vec[i - 1].first;
			const auto curWater = vec[i - 1].second;
			for (int j = 1; j <= i; ++j) {
				for (int k = 1; k <= totalCap; ++k) {
					auto otherVal = ((curCap > k) || (dp[i - 1][j - 1][k - curCap] < 0)) ? -1 :
						std::min(k, dp[i - 1][j - 1][k - curCap] + curWater);
					dp[i][j][k] = std::max(dp[i - 1][j][k], otherVal);
				}
			}
		}
		...
		for (int k = 1; k <= vec.size(); ++k) {
			...
			for (int cap = 0; cap <= totalCap; ++cap) {
				if (dp[vec.size()][k][cap] < 0)
					continue;
				curBest = std::max(curBest, std::min(double(cap), 0.5 * totalWater + 0.5 * dp[vec.size()][k][cap]));
			}
			std::cout << curBest << ' ';
		}
		std::cout << '\n';
