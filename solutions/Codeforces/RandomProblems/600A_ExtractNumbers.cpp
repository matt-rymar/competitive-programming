...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		auto getWord = [&](int idx) -> std::string_view {
			auto end = std::min(sv.find(',', idx), sv.find(';', idx));
			if (end == std::string_view::npos)
				return sv.substr(idx);
			return sv.substr(idx, end - idx);
		};

		std::string a = "\"";
		std::string b = a;
		for (int idx = 0; idx < s.size(); ) {
			auto word = getWord(idx);
			if ((!word.empty()) && ((word.front() != '0') || (word.size() == 1))
				&& std::all_of(word.cbegin(), word.cend(), [](auto c) {return std::isdigit(c); })) {
				a += word;
				a += ',';
			}
			else {
