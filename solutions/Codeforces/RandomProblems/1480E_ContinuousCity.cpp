...
// Assumes R not a power of two
std::vector<int> solveUnshifted(int R)
{
	const auto log = logCeil(R);
	const auto sz = log + 2;
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		R -= offset;
		L -= offset;
		const auto sz = (i64)logCeil(R) + 2;
		//auto fromSource = std::vector<int>();
		//if (!(R & (R - 1))) {
		//	for (int i = sz - 1; i > 1; ++i)
		//		fromSource.push_back(i);
		//}
		//else {
		//	fromSource = solveUnshifted(R);
		//}
		const auto fromSource = solveUnshifted(R);
		std::cout << "YES\n" << sz << ' ' << ((sz - 1) * (sz - 2) / 2) + 1 + fromSource.size() << '\n';
		std::cout << 1 << ' ' << sz << ' ' << offset + 1 << '\n';
		int pow = 1;
		for (auto i : fromSource) {
			std::cout << 1 << ' ' << i << ' ' << offset + pow << '\n';
			pow *= 2;
		}
		for (int i = 2; i < sz; ++i) {
			std::cout << i << ' ' << sz << ' ' << 1 << '\n';
			int pow = 1;
			for (int j = sz - 1; j > i; --j, pow *= 2)
				std::cout << i << ' ' << j << ' ' << pow << '\n';
		}
	}
}