...
std::vector<int64_t> colSuffSums(const std::vector<std::vector<int64_t>>& arr)
{
	std::vector<int64_t> res(arr.front().size());
	res.back() = std::accumulate(arr.cbegin(), arr.cend(), int64_t(0), [](auto acc, const auto& vec) {return acc + vec.back(); });
	for (int i = arr.front().size() - 2; i >= 0; --i) {
		res[i] = res[i + 1] +
			std::accumulate(arr.cbegin(), arr.cend(), int64_t(0), [i](auto acc, const auto& vec) {return acc + vec[i]; });
	}
	return res;
}

int64_t solve(const std::vector<std::vector<int64_t>>& glade)
{
	auto suffSums = colSuffSums(glade);
	int64_t curUpper = glade.back().back(), curLower = glade.front().back();
	for (int i = glade.front().size() - 2; i >= 0; --i) {
		const int64_t finalT = 2 * (glade.front().size() - 1 - i) + 1;
		const auto newUpper = std::max(glade[1][i] + curLower + 2*suffSums[i + 1], 
			curUpper + suffSums[i+1] + finalT*glade[1][i]);
		const auto newLower = std::max(glade[0][i] + curUpper + 2 * suffSums[i + 1],
			curLower + suffSums[i + 1] + finalT * glade[0][i]);
		curUpper = newUpper;
		curLower = newLower;
	}
	return curUpper;
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int n;
		std::cin >> n;
		auto glade = std::vector<std::vector<int64_t>>{ readVectorSz<int64_t>(n), readVectorSz<int64_t>(n) };
		std::cout << solve(glade) << '\n';
	}
}