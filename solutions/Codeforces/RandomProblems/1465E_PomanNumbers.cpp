...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		std::transform(s.cbegin(), s.cend(), std::back_inserter(vals), [](auto c) {return 1LL << (c - 'a'); });
		t -= vals[vals.size() - 1] - vals[vals.size() - 2];
		vals.pop_back(); vals.pop_back();
		std::sort(vals.begin(), vals.end(), std::greater<int64_t>());
		auto curSum = std::accumulate(vals.begin(), vals.end(), 0LL);
		for (auto i : vals) {
			if (curSum - 2 * i >= t)
				curSum -= 2 * i;
		}
