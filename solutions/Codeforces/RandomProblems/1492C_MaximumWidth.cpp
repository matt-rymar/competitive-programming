...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		auto maxSuffixLen = std::vector<int>(s.size());
		maxSuffixLen.back() = (s.back() == t.back());
		for (int i = s.size() - 2; i < s.size(); --i) {
			maxSuffixLen[i] = maxSuffixLen[i + 1];
			if ((maxSuffixLen[i] >= t.size()))
				continue;
			if (s[i] == t[t.size() - 1 - maxSuffixLen[i]])
				++maxSuffixLen[i];
		}
		/* Actually, the task guarantees that that won't happen
		if (maxSuffixLen.front() < t.size()) {
			std::cout << -1 << '\n';
			continue;
		} */
		...
		int curLen = 0;
		int res = 0;
		for (int i = 0; (i < s.size()) && ((curLen + 1) < t.size()); ++i) {
			if (s[i] != t[curLen])
				continue;
			++curLen;
			const auto next = std::upper_bound(maxSuffixLen.cbegin() + i, maxSuffixLen.cend(), t.size() - curLen, std::greater<>());
			res = std::max(res, (int)std::distance(maxSuffixLen.cbegin() + i, next) - 1);
		}
		std::cout << res << '\n';
	}
}