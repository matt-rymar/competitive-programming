...
std::vector<int64_t> prefixCounts(std::string_view sv, const std::vector<int>& zFunc)
{
	auto res = std::vector<int64_t>(sv.size() + 1);
	for (auto i : zFunc)
		++res[i];
...
std::vector<int> prefixoSuffixes(std::string_view sv, const std::vector<int>& zFunc)
{
	std::vector<int> res;
	for (int i = sv.size() - 1; i >= 0; --i) {
		if (i + zFunc[i] == sv.size())
			res.push_back(zFunc[i]);
	}
	return res;
}
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		auto prefs = prefixCounts(s, zFunc);
		auto ps = prefixoSuffixes(s, zFunc);
		std::cout << ps.size() << '\n';
		for (auto i : ps)
			std::cout << i << ' ' << prefs[i] << '\n';
	}
