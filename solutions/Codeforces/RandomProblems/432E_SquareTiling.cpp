...
std::vector<std::string> solve_rec(const int m, const int n, const char above, const char left)
{
	char fill = 'C';
	if ((above != 'B') && (left != 'B'))
		fill = 'B';
	if ((above != 'A') && (left != 'A'))
		fill = 'A';
	auto res = std::vector<std::string>(std::min(m, n), std::string(std::min(m, n), fill));
	if (m < n) { // We're wide
		auto toTheRight = solve_rec(m, n - m, above, fill);
		for (int i = 0; i < m; ++i)
			res[i].append(std::move(toTheRight[i]));
	}
	if (m > n) { // We're tall
		auto below = solve_rec(m - n, n, fill, left);
		for (auto&& s : below)
			res.push_back(s);
	}
	return res;
...
std::vector<std::string> solve(int m, int n)
{
	return solve_rec(m, n, -1, -1);
}
