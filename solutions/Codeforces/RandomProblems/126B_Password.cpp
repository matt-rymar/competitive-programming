...
// For convenience sets z[0] = 0 (though it's easy to change it back since z[0] = sv.size() for arbitrary string)
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		for (int i = 1; i < s.size(); ++i) {
			if (i + z[i] <= s.size())
				longestWithin = std::max(longestWithin, z[i] - (i + z[i] == s.size()));
			if ((i + z[i] == s.size()) && (longestWithin >= z[i])) {
				res = z[i];
				break;
			}
		}
