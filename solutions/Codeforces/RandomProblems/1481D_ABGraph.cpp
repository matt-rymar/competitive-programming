...
std::optional<std::pair<int, int>> findPair(const V<V<char>>& mat) 
{
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		int n, m;
		std::cin >> n >> m;
		auto mat = readMatSz<char>(n, n);

		int x = -1, y = -1;
		if (auto opt = findPair(mat); opt) {
			x = opt->first + 1;
			y = opt->second + 1;
		}
		else if ((m % 2) == 1) {
		...
		if ((mat[0][1] == mat[1][2]) && (mat[1][2] == mat[2][0])) {
			// Cycle
			std::cout << "YES\n";
			for (int i = 0; i <= m; ++i)
				std::cout << (i % 3) + 1 << ' ';
		...
		// Find the topological ordering over the first three vertices
		int a = 0, b = 1, c = 2;
		while (mat[a][b] != mat[a][c]) {
			a = (a + 1) % 3;
			b = (b + 1) % 3;
			c = (c + 1) % 3;
		}
