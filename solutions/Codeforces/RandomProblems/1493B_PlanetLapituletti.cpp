...
constexpr std::array<int, 10> mirror = { 0, 1, 5, -1, -1, 2, -1, -1, 8, -1 };

bool isGood(int time, const int h, const int m)
{
	const auto minutesInDay = h * m;
	time %= minutesInDay;

	const auto hours = time / m;
	const auto mins = time % m;

	for (auto i : { hours, mins }) {
		if ((mirror[i % 10] < 0) || (mirror[i / 10] < 0))
			return false;
	}

	const auto newHours = mirror[mins % 10] * 10 + mirror[mins / 10];
	const auto newMins = mirror[hours % 10] * 10 + mirror[hours / 10];
	return (newHours < h) && (newMins < m);
}

std::string getTime(int time, const int h, const int m)
{
	const auto minutesInDay = h * m;
	time %= minutesInDay;

	return std::string{ char('0' + (time / m) / 10), char('0' + (time / m) % 10), ':', char('0' + (time % m) / 10), char('0' + (time % m) % 10) };
}
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		auto time = std::atoi(s.substr(0, 2).c_str()) * m + std::atoi(s.substr(3, 2).c_str());
		while (!isGood(time, h, m))
			++time;
		std::cout << getTime(time, h, m) << '\n';
	}
}