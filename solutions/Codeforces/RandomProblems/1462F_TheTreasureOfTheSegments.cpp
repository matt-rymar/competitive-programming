...
bool intersect(const std::pair<int, int>& a, const std::pair<int, int>& b)
{
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		begs.reserve(vec.size()); ends.reserve(vec.size());
		for (auto p : vec) {
			begs.push_back(p.first);
			ends.push_back(p.second);
		}
		std::sort(begs.begin(), begs.end());
		std::sort(ends.begin(), ends.end());
		auto res = POS_INF<int>;
		for (auto p : vec) {
			auto toTheLeft = std::distance(ends.cbegin(), std::lower_bound(ends.cbegin(), ends.cend(), p.first));
			auto toTheRight = std::distance(std::upper_bound(begs.cbegin(), begs.cend(), p.second), begs.cend());
			auto intersect = vec.size() - (toTheLeft + toTheRight);
			if ((vec.size() - intersect) < res)
				res = vec.size() - intersect;
		}
		std::cout << res << '\n';
	}
}