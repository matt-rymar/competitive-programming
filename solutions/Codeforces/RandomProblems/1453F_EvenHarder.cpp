...
int main()
{
	...
	std::cin >> t;
	for (int i = 0; i < t; ++i) {
		const auto vec = readVec<int>();

		auto dp = std::vector<std::vector<int>>(vec.size(), std::vector<int>(vec.size(), vec.size()));
		for (int i = 0; i < dp[0].size(); ++i)
			dp[0][i] = 0;
		for (int i = 1; i < vec.size(); ++i) {
			int count = 0; // No of guys between us and i that can reach i
			for (int j = i - 1; j >= 0; --j) {
				if (j + vec[j] < i)
					continue;
				dp[i][j + vec[j]] = std::min(dp[i][j + vec[j]], dp[j][i - 1] + count);
				++count;
			}
