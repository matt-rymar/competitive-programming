...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int a, b, c;
		std::cin >> a >> b >> c;
		const auto min = std::min({ a, b, c });
		const auto diff = a + b + c - 3 * min;
		const auto fullRounds = (diff - 1) / 6;
		const auto rem = diff % 6;
		if ((min <= fullRounds) || ((rem % 3) != 0)) {
			std::cout << "NO\n";
			continue;
		}
		a = b = c = min - fullRounds;
		if (((fullRounds == 0) && (((diff == 0) && (a % 3 == 0)) || ((diff == 3) && ((a % 3) == 2)) || ((diff == 6) && ((a % 3) == 1))))
			|| ((fullRounds > 0) && (((rem == 0) && ((a % 3) == 1)) || ((rem == 3) && ((a % 3) == 2)))))
			std::cout << "YES\n";
		else
			std::cout << "NO\n";
	}
