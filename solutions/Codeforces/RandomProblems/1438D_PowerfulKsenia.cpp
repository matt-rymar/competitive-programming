...
bool possible(const std::vector<uint32_t>& vec)
{
	if (vec.size() & 1)
		return true;
	return std::accumulate(vec.cbegin(), vec.cend(), uint32_t(0), [](auto acc, auto el) {return acc ^ el; }) == 0;
}

void outputOps(const std::vector<uint32_t>& vec)
{
	for (int i = 0; i < (vec.size() - 1); i += 2)
		std::cout << i + 1 << ' ' << i + 2 << ' ' << vec.size() << '\n';
}
