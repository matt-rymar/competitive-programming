...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		auto freqs = std::vector<int>(200'005);
		for (auto i : vec)
			++freqs[i];

		auto cumFreqs = std::vector<int>();
		std::partial_sum(freqs.cbegin(), freqs.cend(), std::back_inserter(cumFreqs));

		auto dp = std::vector<int>(200'005);
		for (int i = 200'000; i >= 1; --i) {
			dp[i] = cumFreqs.back() - cumFreqs[i];
			for (int j = 2 * i; j < dp.size(); j += i)
				dp[i] = std::min(dp[i], dp[j] + cumFreqs[j - 1] - cumFreqs[i]);
		}
		std::cout << dp[1] << '\n';
	}
}