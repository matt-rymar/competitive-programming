...
int main()
{
	...
	//std::cin >> t;
	for (int i = 0; i < t; ++i) {
		auto ns = readVecSz<int>(3);
		std::vector<std::vector<i64>> bags;
		for (int i = 0; i < ns.size(); ++i)
			bags.push_back(readVecSz<i64>(ns[i]));

		std::vector<i64> mins;
		std::vector<i64> sums;
		i64 total = 0;
		for (int i = 0; i < bags.size(); ++i) {
			mins.push_back(*std::min_element(bags[i].cbegin(), bags[i].cend()));
			sums.push_back(std::accumulate(bags[i].cbegin(), bags[i].cend(), i64(0)));
			total += sums.back();
		}

		auto best = NEG_INF<i64>;
		for (int i = 0; i < bags.size(); ++i) {
			// Case 1: we use this bag as the "all-negatives dump"
			best = std::max(best, total - 2 * sums[i]);

			// Case 2: We use an element of this bag as the root and select one element each from two bags as dump
			// (Note that since bag i is the root, it can only be selected as dump if it has at least two elements)
			for (int j = 0; j < bags.size(); ++j) {
				if ((j == i) && (bags[i].size() < 2))
					continue;
				for (int k = j + 1; k < bags.size(); ++k) {
					if ((k == i) && (bags[i].size() < 2))
						continue;
					best = std::max(best, total - 2*mins[j] - 2*mins[k]);
				}
			}
		}
