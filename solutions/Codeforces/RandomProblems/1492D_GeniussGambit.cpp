...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		i64 a, b, k;
		std::cin >> a >> b >> k;
		--b; // Since both numbers have to start with a 1

		const auto n = a + b + 1;
		if (k == 0) {
			const auto str = std::string(b + 1, '1') + std::string(a, '0');
			std::cout << "Yes\n" << str << '\n' << str << '\n';
			continue;
		}
		if ((k > (n - 2)) || (b == 0) || (a == 0)) {
		...
		auto x = std::string("1") + std::string(n - 1, '0');
		auto y = x;
		y.back() = '1';
		const auto funkyIdx = x.size() - 1 - k;
		x[funkyIdx] = '1';
		int cnt = 2;
		int i = y.size() - 2;
		for (; (i > funkyIdx) && (cnt <= b); --i, ++cnt)
			x[i] = y[i] = '1';
		for (--i; cnt <= b; --i, ++cnt)
			x[i] = y[i] = '1';
		std::cout << "Yes\n" << x << '\n' << y << '\n';
