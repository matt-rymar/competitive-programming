...
struct Data
{
	...
	{
		data.zeroCount = upd.toZero ? data.rangeLen : 0;
	}
};
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		int n, q;
		std::cin >> n >> q;
		auto start = readVecSz<char>(n);
		auto end = readVecSz<char>(n);
		auto queries = readVecSz<P<int, int>>(q);

		bool works = true;
		auto tree = DefSegmentTree([&](int i) -> Data {return Data{ .zeroCount = (end[i] == '0'), .rangeLen = 1 }; }, end.size());

		for (auto it = queries.crbegin(); it != queries.crend(); ++it) {
			auto [lo, hi] = *it;
			--lo; --hi;
			const auto data = tree.query(lo, hi);
			if (2 * data.zeroCount == data.rangeLen) {
				works = false;
				break;
			}
			if (2 * data.zeroCount > data.rangeLen)
				tree.update(lo, hi, UpdateData{ true });
			else
				tree.update(lo, hi, UpdateData{ false });
		}

		auto out = tree.getContents();
		for (int i = 0; i < out.size(); ++i) {
			if (out[i].zeroCount != (start[i] == '0')) {
				works = false;
				break;
			}
		}
