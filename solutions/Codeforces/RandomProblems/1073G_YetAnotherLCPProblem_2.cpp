...
// Allows us to provide a convenient default for the RMQ operation while allowing the use of a general idempotent operation
template <typename T>
struct MinWrapper
{
...
public:
	...
	{
		fillLogs(vec.size() + 1);
		mins = std::vector<std::vector<T>>(logs[vec.size()] + 1, std::vector<T>(vec.size())),
			mins[0] = std::move(vec);
...
// Computes the suffix array as well as the LCP array of an arbitrary string in O(nlgn) time
// Afterwards can answer LCP queries about two arbitrary suffixes in O(1) time
//	(with O(nlgn) precomputation time)
// Note that LCP[i] stores the LCP of i-th and (i+1)th suffix, not i and (i-1)th as is also done sometimes
...
private:
	...
	std::vector<int> suffixLogSquared(std::string_view sv, Mode mode) const
	{
		...
		// Since our string is cirular, we may have negative or large indices, which the function below handles
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		auto sa = SuffixArray(s, SuffixArray::Mode::endSmall);
		for (int i = 0; i < q; ++i) {
			int k, l;
			std::cin >> k >> l;
