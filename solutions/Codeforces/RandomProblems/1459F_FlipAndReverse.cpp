...
std::pair<int, int> minMaxPos(std::string_view sv)
{
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		const auto [minPos, maxPos] = minMaxPos(s);
		const auto size = maxPos - minPos + 1;

		// Construct the graph
		auto left = std::vector<int>(size);
		auto right = left;
		int pos = -minPos;
		for (auto c : s) {
			if (c == '0') {
		...
		// Do optimal Euler Walk on the graph
		std::string res;
		for (int pos = -minPos; left[pos] || right[pos]; ) {
			if ((left[pos] == 0) || ((right[pos] > 0) && (left[pos + 1] > 0))) {
				--right[pos++];
				res.push_back('0');
			}
			else {
