...
std::vector<i64> getPathCounts(const int n, const int k)
{
	auto dp = std::vector<std::vector<i64>>(k + 1, std::vector<i64>(n, 1));
	for (int i = 1; i <= k; ++i) {
		dp[i][0] = dp[i - 1][1];
		dp[i].back() = dp[i - 1][dp[i].size() - 2];
		for (int j = 1; (j + 1) < dp[i].size(); ++j)
	...
	for (int i = 0; i <= k; ++i) {
		for (int j = 0; j < dp[i].size(); ++j)
			res[j] = (res[j] + ((dp[i][j] * dp[k - i][j]) % MOD)) % MOD;
	}
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		auto counts = getPathCounts(n, k);
		auto sum = std::inner_product(vec.cbegin(), vec.cend(), counts.cbegin(), i64(0),
			[](auto acc, auto val) {return (acc + val) % MOD; }, [](auto val1, auto val2) {return (val1 * val2) % MOD; });
		for (int i = 0; i < q; ++i) {
			int idx;
			i64 x;
			std::cin >> idx >> x;
			--idx;
			sum = (MOD + sum - ((vec[idx] * counts[idx]) % MOD)) % MOD;
			vec[idx] = x;
			sum = (sum + ((vec[idx] * counts[idx]) % MOD)) % MOD;
			std::cout << sum << '\n';
		}
	}
}