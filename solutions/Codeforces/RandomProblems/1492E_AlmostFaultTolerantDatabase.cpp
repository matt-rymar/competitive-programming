...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		auto mat = readMat<int>();

		auto getDiffs = [&](int idx) {
			std::vector<int> res;
			for (int i = 0; (i < mat[idx].size()) && (res.size() < 5); ++i) {
				if (mat[0][i] != mat[idx][i])
					res.push_back(i);
			}
			return res;
		};
		auto solve = y_combinator([&](auto self, const int idx, const int changesRem, const int locked) -> std::optional<std::vector<std::pair<int, int>>> {
			if (idx >= mat.size())
				return std::vector<std::pair<int, int>>{};
			auto diffs = getDiffs(idx);
			if (diffs.size() <= 2)
				return self(idx + 1, changesRem, locked);
			if ((diffs.size() > 4) || (changesRem <= 0))
				return std::nullopt;
			if (diffs.size() == 3) {
				for (auto i : diffs) {
					if (i == locked)
						continue;
					const auto old = mat[0][i];
					mat[0][i] = mat[idx][i];
					if (auto res = self(1, changesRem - 1, i); res) {
						res->emplace_back(i, mat[idx][i]);
						return res;
					}
					mat[0][i] = old;
				}
			...
			for (int i = 0; i < diffs.size(); ++i) {
				for (int j = i + 1; j < diffs.size(); ++j) {
					const auto old1 = mat[0][diffs[i]];
					const auto old2 = mat[0][diffs[j]];
					mat[0][diffs[i]] = mat[idx][diffs[i]];
					mat[0][diffs[j]] = mat[idx][diffs[j]];
					if (auto res = self(1, 0, -1); res) {
						res->emplace_back(diffs[i], mat[idx][diffs[i]]);
						res->emplace_back(diffs[j], mat[idx][diffs[j]]);
						return res;
					}
					mat[0][diffs[i]] = old1;
					mat[0][diffs[j]] = old2;
		...
		if (auto res = solve(1, 2, -1); res) {
			std::cout << "Yes\n";
			for (int i = 0; i < res->size(); ++i)
				mat[0][(*res)[i].first] = (*res)[i].second;
			std::cout << mat[0] << '\n';
		}
		else {
			std::cout << "No\n";
