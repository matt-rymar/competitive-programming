...
int solve(const VVI& adj)
{
	auto topSortOpt = topologicalSort(adj);
	if (!topSortOpt)
		return -1;

	auto dp = std::vector<int>(adj.size(), 1);

	for (auto it = topSortOpt->crbegin(); it != topSortOpt->crend(); ++it) {
		for (auto j : adj[*it]) {
			const auto cost = dp[j] + (j < (*it));
			dp[*it] = std::max(dp[*it], cost);
		}
	}
