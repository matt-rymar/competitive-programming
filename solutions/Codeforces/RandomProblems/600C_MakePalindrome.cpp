...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		auto counts = std::vector<int>(26);
		for (auto c : s)
			++counts[c - 'a'];
		int oddCounts = std::count_if(counts.cbegin(), counts.cend(), [](auto i) {return (i % 2) != 0; });
		int odds = counts.size() - 1;
		auto advanceOdds = [&]() {while ((odds >= 0) && ((counts[odds] % 2) == 0)) --odds; };
		advanceOdds();
		char oddChar = -1;
		for (int i = 0; i < 26; ++i) {
			if (!(counts[i] & 1))
