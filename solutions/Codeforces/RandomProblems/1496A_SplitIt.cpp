...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		const auto isPalindrome = [](std::string_view sv, const int depth) {
			for (int i = 0; i < std::min(depth, (int)sv.size() / 2); ++i) {
				if (sv[i] != sv[sv.size() - 1 - i])
					return false;
			}
			return true;
		};

		if ((n < (2 * k + 1)) || (!isPalindrome(s, k)))
			std::cout << "NO\n";
		else 
			std::cout << "YES\n";
