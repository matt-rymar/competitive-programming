...
//  -- Codeforces 474F--Ant Colony (very simple with no updates)
//	-- Codeforces 1439C--Greedy Shopping
//	-- Codeforces 1469F--Power Sockets
//	-- Codeforces 1478E--Nezzar And Binary String
template<typename Data, typename UpdateData>
class SegmentTree
{
public:
	// Constructs the segment tree on [0, sz) in O(nlgn) time
...
struct UpdateData
{
	...
	// Combines two updates if they arrive at the same Node
	friend UpdateData& operator+=(UpdateData& upd, const UpdateData& rhs)
	{
		upd.doUpdate = upd.doUpdate ^ rhs.doUpdate;
		return upd;
	}
};
...
struct Data
{
	...
	{
		Data res{ .leftFirst = left.leftFirst,
				.leftSecond = left.leftSecond,
				.rightFirst = right.rightFirst,
				.rightSecond = right.rightSecond,
				.longestWithin = std::max(left.longestWithin, right.longestWithin),
				.longestOpposite = std::max(left.longestOpposite, right.longestOpposite),
				.rangeLen = left.rangeLen + right.rangeLen };

		// Update lefts
		if (std::abs(left.leftFirst) == left.rangeLen) {
			if (left.leftFirst * right.leftFirst > 0) {
				// >>>>||>>><<<<...
				// or <<<<||<<<>>>>...
				res.leftFirst += right.leftFirst;
				res.leftSecond = right.leftSecond;
			}
			else {
				// >>>>||<<<>>>>...
				// or <<<<||>>><<<<...
				res.leftSecond = right.leftFirst;
			}
		}
		else if ((std::abs(left.leftFirst) + std::abs(left.leftSecond)) == left.rangeLen) {
			// >>>><<<<||>>><<<<...
			// or <<<<||<<<>>>>...
			if (left.leftSecond * right.leftFirst > 0)
				res.leftSecond += right.leftFirst;
		}

		// Update rights
		if (std::abs(right.rightFirst) == right.rangeLen) {
			if (left.rightFirst * right.rightFirst > 0) {
				res.rightFirst += left.rightFirst;
				res.rightSecond = left.rightSecond;
			}
			else {
				res.rightSecond = left.rightFirst;
			}
		}
		else if ((std::abs(right.rightFirst) + std::abs(right.rightSecond)) == right.rangeLen) {
			if (right.rightSecond * left.rightFirst > 0)
				res.rightSecond += left.rightFirst;
		}

		// Finally, check if a new better sequence exists at the boundary
		const auto overlapLen = std::abs(left.rightFirst) + std::abs(right.leftFirst);
		if (left.rightFirst * right.leftFirst > 0) {
			if (left.rightFirst < 0) {
				// ...>><<<||<<<>>>...
				res.longestWithin = std::max(res.longestWithin, std::abs(left.rightSecond) + overlapLen);
				res.longestOpposite = std::max(res.longestOpposite, std::abs(right.leftSecond) + overlapLen);
			}
			else {
				// ...<<>>>||>>><<<...
				res.longestWithin = std::max(res.longestWithin, std::abs(right.leftSecond) + overlapLen);
				res.longestOpposite = std::max(res.longestOpposite, std::abs(left.rightSecond) + overlapLen);
			}
		}
		else {
			if (left.rightFirst < 0) {
				// ...>><<<||>>><<<...
				res.longestOpposite = std::max(res.longestOpposite, overlapLen);
			}
			else {
				// ...<<>>>||<<<>>>...
				res.longestWithin = std::max(res.longestWithin, overlapLen);
			}
		}
	...
	friend Data& operator+=(Data& data, const UpdateData& upd)
	{
		...
		data.rightFirst *= -1;
		data.rightSecond *= -1;
		std::swap(data.longestOpposite, data.longestWithin);
		return data;
	}
};
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		for (int i = 0; i < q; ++i) {
			...
			std::cin >> l >> r;
			--l; --r;
			tree.update(l, r, UpdateData{ true });
			//for (auto d : tree.getContents())
			//	std::cout << ((d.leftFirst < 0) ? '<' : '>');
			//std::cout << '\n';
			std::cout << tree.query(l, r).longestWithin << '\n';
		}
	}
}