	...
	public:
		...
		Data query(const int lo, const int hi)
		{
			...
			if (coversUs(lo, hi))
				return data;
			push();
			const auto leftAcc = left->query(lo, hi); // Make sure to query the left segment firsts
...
int solve(const std::vector<int>& chains, const int k)
{
	if (k == 1)
		return 0;
	const auto prefSumComp = [](const Data& lhs, const Data& rhs) { return lhs.sum < rhs.sum; };
	const auto treeSz = chains.front() + chains.size() + 10;
	auto depths = DefSegmentTree([](int i) {return Data{ 0, 1 }; }, treeSz);
	depths.pointUpdate(0, UpdateData{ 1 });
	int curDepth = 0;
	int noAtDepth = 1;
	int best = -1;
	for (auto i : chains) {
		depths.pointUpdate(curDepth, UpdateData{ -1 });
		depths.update(curDepth + 2, curDepth + 1 + i / 2, UpdateData{ 1 });
		depths.update(curDepth + 2, curDepth + 1 + i / 2 - !(i & 1), UpdateData{ 1 });
		auto val = depths.lowerBound(0, Data{ .sum = k, .rangeLen = -1 }, prefSumComp);
		if ((val < treeSz) && ((best == -1) || (val < best)))
			best = val;
		if ((--noAtDepth) > 0)
			continue;
		while ((noAtDepth = depths.pointQuery(++curDepth).sum) == 0)
			;
	}
