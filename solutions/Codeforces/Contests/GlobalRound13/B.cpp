		...
		auto firstWall = [&]() {
			...
			for (; (i + 1) < vec.size(); ++i) {
				if (vec[i] == 0) {
					...
					else if (i == (zeroChain + 1))
						zeroChain = i;
				}
				if (std::abs(vec[i + 1] - vec[i]) > 1)
					break;
				if ((std::abs(vec[i + 1] - vec[i]) == 1) && ((zeroSpot < 0) || ((i == zeroSpot))))
					sideways = true;
			}
			if ((vec[i] == 0) && (zeroSpot < 0))
				zeroSpot = zeroChain = i;
			if ((zeroSpot >= 0) || ((i + 1) == vec.size())) {
				auto cost = std::min(2*v, u + v);
				if (sideways)
					cost = std::min(u, v);
				else if ((i + 1) < vec.size())
					cost = std::min(cost, u * (zeroChain - zeroSpot + 1));
				return cost;
			}
			return (i64)-1;
		};
		...
		auto secWall = [&]() {
			...
			if ((zeroSpot >= 0) && (i > 0)) {
				...
				if (sideways)
					...
					cost = std::min(cost, u * (zeroSpot - zeroChain + 1));
				return cost;
			}
			return (i64)-1;
		};

		if (auto val = secWall(); val > 0)
			res += val;
