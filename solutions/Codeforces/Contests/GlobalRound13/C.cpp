...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		auto tree = DefSegmentTree([&](auto i) {return Data{ .sum = vec[i], .rangeLen = 1 }; }, vec.size());
		i64 jumps = 0;
		for (int i = 0; i < vec.size(); ++i) {
			const auto actualValue = tree.pointQuery(i).sum;
			const auto actualJumps = std::max(i64(0), actualValue - 1);
			jumps += actualJumps;
			if ((vec[i] > 1) && ((i+2) < vec.size()))
				tree.update(i + 2, std::min(i + vec[i], (i64)vec.size() - 1), UpdateData{ -1 });
			if ((actualValue < 1) && ((i+1) < vec.size()))
				tree.update(i + 1, i + 1, UpdateData{ actualValue - 1 });
		}
		std::cout << jumps << '\n';
	}
}