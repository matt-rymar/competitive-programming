...
void updateVec(std::vector<i64>& left, const std::vector<i64>& right, const std::vector<P<int, int>>& adj)
{
	...
	for (auto [i, j] : adj)
		(coolStuff[right[j]][i])++;
	while ((!counts.empty()) && (counts.cbegin()->first < 0))
		counts.erase(counts.begin());
	while ((!coolStuff.empty()) && (coolStuff.cbegin()->first < 0))
		coolStuff.erase(coolStuff.begin());

	const auto init = (counts.empty()) ? -1 : counts.cbegin()->first;
	auto best = V<i64>(left.size(), init);
	for (auto it = coolStuff.cbegin(); it != coolStuff.cend(); ++it) {
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		int n1, n2, n3, n4;
		std::cin >> n1 >> n2 >> n3 >> n4;
		auto as = readVecSz<i64>(n1);
		auto bs = readVecSz<i64>(n2);
		auto cs = readVecSz<i64>(n3);
		auto ds = readVecSz<i64>(n4);

		int m1;
		std::cin >> m1;
		auto edge1 = readVecSz<std::pair<int, int>>(m1, [](const auto& p) {return std::make_pair(p.first - 1, p.second - 1); });
		int m2;
		std::cin >> m2;
		auto edge2 = readVecSz<std::pair<int, int>>(m2, [](const auto& p) {return std::make_pair(p.first - 1, p.second - 1); });
		int m3;
		std::cin >> m3;
		auto edge3 = readVecSz<std::pair<int, int>>(m3, [](const auto& p) {return std::make_pair(p.first - 1, p.second - 1); });

		updateVec(cs, ds, edge3);
		updateVec(bs, cs, edge2);
		updateVec(as, bs, edge1);
