...
int main()
{
	...
	std::cin >> t;
	for (int k = 0; k < t; ++k) {
		const auto vec = readVec<std::pair<int64_t, int64_t>>();
		int64_t pos = 0;
		int successful = 0;
		for (int i = 0; i < vec.size(); ) {
			...
			if (vec[i].second < pos)
				dirLeft = true;
			const int64_t dirMult = dirLeft ? -1 : 1;
			const auto until = vec[i].first + std::abs(pos - vec[i].second);
			if (((i + 1) >= vec.size()) || (vec[i + 1].first >= until))
				++successful;
			auto curPos = pos;
			int j = i + 1;
			for (; (j < vec.size()) && (vec[j].first < until); ++j) {
				curPos += (vec[j].first - vec[j - 1].first) * dirMult;
				auto one = curPos, two = curPos;
				if ((j + 1) >= vec.size())
					two = vec[i].second;
				else
					two += (vec[j + 1].first - vec[j].first) * dirMult;
				auto left = std::min(one, two);
				auto right = std::max(one, two);
				if ((left <= vec[j].second) && (vec[j].second <= right))
					++successful;
			}
			pos = vec[i].second;
			i = j;
