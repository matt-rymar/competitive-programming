...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		int n;
		std::cin >> n;
		auto vec = readVecSz<P<i64, i64>>(n);
		auto ts = readVecSz<i64>(n);

		i64 curTime = vec[0].first + ts[0];
		for (int i = 0; (i+1) < vec.size(); ++i) {
			const auto [arr, dep] = vec[i];
			const auto minWait = (dep - arr + 1) / 2;

			curTime = std::max(curTime, arr);
			curTime += minWait;
			curTime = std::max(curTime, dep);
			const auto travelTime = vec[i + 1].first - dep + ts[i+1];
			curTime += travelTime;
		}
		curTime = std::max(curTime, vec.back().first);
		std::cout << curTime << '\n';
	}
}