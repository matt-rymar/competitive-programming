...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int n;
		std::cin >> n;
		auto vec = readVecSz<i64>(2 * n);

		auto cur = std::multiset<i64, std::greater<i64>>(vec.begin(), vec.end());

		std::vector<std::pair<i64, i64>> ops;
		auto cool = [&](auto cur, i64 curSum) {
			while (cur.size() > 1) {
				auto i = *cur.begin();
				cur.erase(cur.begin());
				auto it = cur.find(curSum - i);
				if (it == cur.end())
					return false;
				ops.emplace_back(i, *it);
				curSum = i;
				cur.erase(it);
			}
			return true;
