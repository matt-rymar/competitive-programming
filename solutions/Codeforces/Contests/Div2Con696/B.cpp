...
auto isPrime = std::vector<bool>(200'005, true);
...
std::vector<int> primeSieve(const int n)
{
	...
	for (int64_t i = 3; i < isPrime.size(); i += 2) { // Technically i*i <= n would be enough to find all primes, but since we need to also add primes > sqrt(n) into the vector, might as well do it in this loop
		if (!isPrime[i])
			continue;
		res.push_back(i);
...
const auto primes = primeSieve(200'001);
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		auto firstPrimeAfter = [&](auto i) -> i64 {while (!isPrime[i]) ++i; return i; };
		auto sec = firstPrimeAfter(d + 1);
		auto third = firstPrimeAfter(sec + d);
		std::cout << sec * third << '\n';
	}
}