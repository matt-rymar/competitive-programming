...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		int n, m;
		std::cin >> n >> m;
		auto initial = readVecSz<int>(n);
		auto end = readVecSz<int>(n);
		auto cols = readVecSz<int>(m);

		auto needs = std::vector<std::vector<int>>(n);
		for (int i = 0; i < n; ++i) {
			if (initial[i] != end[i])
				needs[end[i]-1].push_back(i);
		}
        
		int lastIdx = -1;
		if (needs[cols.back() - 1].size() > 0) {
			lastIdx = needs[cols.back() - 1].back();
			needs[cols.back() - 1].pop_back();
		}
		else {
			auto it = std::find(end.cbegin(), end.cend(), cols.back()); 
			if (it != end.cend())
				lastIdx = std::distance(end.cbegin(), it);
		}
		if (lastIdx < 0) {
			std::cout << "NO\n";
			continue;
		...
		for (int i = 0; (i+1) < m; ++i) {
			auto col = cols[i] - 1;
			if (needs[col].size() > 0) {
				sol.push_back(needs[col].back() + 1);
				needs[col].pop_back();
			}
			else {
				sol.push_back(lastIdx + 1);
			}
		}
		bool bad = false;
