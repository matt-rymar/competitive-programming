...
void fillLookup()
{
	lookup = std::vector<std::vector<bool>>(10, std::vector<bool>(100, false));
	for (int i = 1; i <= 9; ++i) {
		for (int x = i; x < 100; x += i)
			lookup[i][x] = true;
		for (int m = 10; m < 100; m += 10) {
			for (int x = m + i; x < 100; x += i)
				lookup[i][x] = true;
		}
	}
}
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		int q, d;
		std::cin >> q >> d;
		auto vec = readVecSz<int>(q);

		auto isGood = [&](int x, int d) -> bool {
			if (std::to_string(x).find('0' + d) != std::string::npos)
				return true;
			const auto rem = x % d;
			if ((rem == 0) || ((10 * d + rem) <= x))
				return true;
