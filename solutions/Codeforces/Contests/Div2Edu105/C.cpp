...
i64 solve(const std::vector<i64>& boxes, const std::vector<i64>& special)
{
	if (boxes.empty() || special.empty())
		return 0;
	std::vector<i64> suffCount(boxes.size(), 0);
	int spIdx = special.size() - 1;
	int curCount = 0;
	for (int i = boxes.size() - 1; i >= 0; --i) {
		while ((spIdx >= 0) && (special[spIdx] > boxes[i]))
			--spIdx;
		if ((spIdx >= 0) && (special[spIdx] == boxes[i]))
			++curCount;
		suffCount[i] = curCount;
	}
	// Initial case: no boxes moved;
	i64 res = suffCount[0];

	for (int i = 0; i < special.size(); ++i) {
		auto end = special[i];
		auto it = std::upper_bound(boxes.cbegin(), boxes.cend(), end);
		i64 noBoxes = std::distance(boxes.cbegin(), it);
		if (noBoxes <= 0)
			continue;
		auto firstSpecialIdx = std::distance(special.cbegin(), std::lower_bound(special.cbegin(), special.cend(), end - noBoxes + 1));
		i64 noSpecials = i - firstSpecialIdx + 1;
		auto val = std::min(noSpecials, noBoxes);
		if (noBoxes < boxes.size())
			val += suffCount[noBoxes];
		res = std::max(res, val);
	}
	return res;
}
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		int n, m;
		std::cin >> n >> m;
		auto boxes = readVecSz<i64>(n);
		auto special = readVecSz<i64>(m);

		std::vector<i64> posB, negB, posSp, negSp;
		for (auto i : boxes) {
			if (i < 0)
				negB.push_back(-i);
			else
		...
		std::reverse(negB.begin(), negB.end());
		for (auto i : special) {
			if (i < 0)
				negSp.push_back(-i);
			else
		...
		std::reverse(negSp.begin(), negSp.end());

		auto res = solve(posB, posSp) + solve(negB, negSp);
		std::cout << res << '\n';
	}
}