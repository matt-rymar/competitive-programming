...
std::vector<int64_t> prefixSums(const V<i64>& vec)
{
	V<i64> res{ 0 };
...
void sums(const V<i64>& vec, const V<i64>& prefs, int l, int r, std::unordered_set<int64_t>& res)
{
	if (l > r)
		return;
	if (l == r) {
	...
	res.insert(prefs[r + 1] - prefs[l]);
	auto mid = (vec[r] + vec[l]) / 2;
	auto pos = std::distance(vec.cbegin(), std::upper_bound(vec.cbegin(), vec.cend(), mid));
	if ((pos == r + 1) || (pos == l))
		return;
	sums(vec, prefs, l, pos - 1, res);
	sums(vec, prefs, pos, r, res);
}

std::unordered_set<i64> realSums(const V<i64>& vec)
{
	auto prefs = prefixSums(vec);
	std::unordered_set<i64> res;
	sums(vec, prefs, 0, vec.size() - 1, res);
	return res;
}
