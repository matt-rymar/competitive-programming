...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		i64 k, l, r, t, x, y;
		std::cin >> k >> l >> r >> t >> x >> y;
		int64_t badLeft = std::max(l, r - y + 1);
		int64_t badRight = std::min(r, l + x - 1);
		int64_t alsoBad = l - y + x - 1;
		int64_t days = 0;
		if (y < x) {
			int64_t val = k;
			if ((val + y > r) && (val - x < l))
				...
				const auto dailyDiff = y - x;
				auto worstRight = alsoBad;
				if ((badLeft <= badRight) && (val >= badLeft))
					worstRight = std::max(worstRight, badRight);
				days += (std::max(i64(0), val - worstRight) + dailyDiff - 1) / dailyDiff;
			}
		}
		else if (y == x) {
			if ((badLeft <= k) && (k <= badRight))
				days = 0;
			else
				days = t;
		}
			...
			auto maxDec = (y + x - 1) / x;
			auto effDec = maxDec - y;
			auto oneDiff = y - x;
			auto init = (std::max(i64(0), r - y + 1 - k) + oneDiff - 1) / oneDiff;
			days += init;
			k += init * oneDiff;
			auto possible = [&](i64 headroom) -> bool {return (k - (y - headroom + x - 1) / x * x) >= l; };
			bool goodHeadroomFound = true;
			if (possible(r - k)) {
				auto curVisited = k - effDec;

			}
			else {
				days += (k - alsoBad + x - 1) / x;
			}
		}
		if (days >= t)
			std::cout << "Yes\n";
