...
constexpr int64_t inv = 499122177;
...
int main()
{
	...
	std::cin >> t;
	for (int i = 0; i < t; ++i) {
		auto vec = readVector<int64_t>();
		auto sum = std::accumulate(vec.cbegin(), vec.cend(), int64_t(0));
		auto max = *std::max_element(vec.cbegin(), vec.cend());
		if (sum > (max * (vec.size() - 1)))
			std::cout << (vec.size() - 1 - (sum % (vec.size() - 1))) % (vec.size() - 1) << '\n';
		else
			std::cout << ((vec.size() - 1) * max) - sum << '\n';
	}
}