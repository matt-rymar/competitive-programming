...
int64_t maxWithin(const std::vector<int64_t>& vec, int64_t x, int64_t k)
{
	int64_t res = 0;
	for (int i = 0; i <= x; ++i) {
		res += vec[i] << i;
		if (res >= k) // Avoid overflow (could still happen, maybe, not sure)
			return k;
	}
	return res;
}

int64_t pastUs(const std::vector<int64_t>& vec, int64_t x, int64_t k)
{
	if (k == 0)
		return 0;
	if (x >= vec.size())
	...
	if (vec[x] >= k)
		return 0;
	auto rem = k - vec[x];
	auto above = pastUs(vec, x + 1, (rem + 1) / 2);
	if (above < 0)
		return -1;
	return above + (rem + 1) / 2;
}

int64_t solve(const std::vector<int64_t>& vec, int64_t x, int64_t k)
{
	auto curMax = maxWithin(vec, x, k);
	auto cur = std::accumulate(vec.cbegin(), vec.cbegin() + x + 1, int64_t(0));
	if (cur >= k)
		return 0;
	const auto maxAfterSplits = int64_t(1) << x;
	int64_t ops = 0;
	for (int i = x + 1; i < vec.size(); ++i) {
		const auto overhead = i - x - 1;
		const auto maxGen = int64_t(1) << (i - x);
		if ((curMax >= k) && ((k - cur) <= overhead))
			return (k - cur) + ops;
		for (int64_t used = 0; used < vec[i]; ++used) {
			if (k - cur >= maxGen) {
				curMax = std::min(k, curMax + maxGen * maxAfterSplits);
				cur += maxGen;
				ops += maxGen - 1;
				continue;
			}
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int n, q;
		std::cin >> n >> q;
		auto vec = readVectorSz<int64_t>(n);
		for (int i = 0; i < q; ++i) {
			int t;
			std::cin >> t;
			if (t == 1) {
