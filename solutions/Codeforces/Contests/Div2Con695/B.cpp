...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		int val = 0;
		int bestFix = 0;
		auto isBad = [&](int i) -> int {if ((i == 0) || (i == (vec.size() - 1))) return false; return ((vec[i - 1] < vec[i]) && (vec[i + 1] < vec[i])) || ((vec[i - 1] > vec[i]) && (vec[i + 1] > vec[i])); };
		for (int i = 1; (i + 1) < vec.size(); ++i) {
			if ((vec[i - 1] < vec[i]) && (vec[i + 1] < vec[i])) {
				++val;
				int curFix = 1;
				auto badNeighbours = (int)isBad(i - 1) + (int)isBad(i + 1);
				auto old = vec[i];
				vec[i] = vec[i - 1];
				curFix = std::max(curFix, 1 + badNeighbours - ((int)isBad(i - 1) + (int)isBad(i + 1)));
				vec[i] = vec[i + 1];
				curFix = std::max(curFix, 1 + badNeighbours - ((int)isBad(i - 1) + (int)isBad(i + 1)));
				vec[i] = old;
				bestFix = std::max(curFix, bestFix);
			}
