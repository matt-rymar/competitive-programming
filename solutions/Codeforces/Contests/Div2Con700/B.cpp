...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		i64 A, B, n;
		std::cin >> A >> B >> n;
		auto attack = readVecSz<i64>(n);
		auto hp = readVecSz<i64>(n);

		auto toKill = std::vector<i64>();
		for (int i = 0; i < attack.size(); ++i)
			toKill.push_back(attack[i] * ((hp[i] + A - 1) / A));
		auto overall = std::accumulate(toKill.begin(), toKill.end(), (i64)0);
		bool possible = false;
		for (int i = 0; i < n; ++i) {
			auto rem = B - (overall - toKill[i]);
			if (rem < 1)
				continue;
			auto hits = ((hp[i] + A - 1) / A) - 1;
			if ((rem - hits * attack[i]) > 0) {
				possible = true;
				break;
			}
		}
