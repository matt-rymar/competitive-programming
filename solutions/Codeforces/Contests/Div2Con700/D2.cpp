...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		auto cols = VVI(maxCol);
		for (int i = 0; i < vec.size(); ++i) {
			const auto col = vec[i] - 1;
			cols[col].push_back(i);
		}
		auto next = VI(vec.size(), vec.size());
		for (int i = 0; i < cols.size(); ++i) {
			for (int j = 0; (j + 1) < cols[i].size(); ++j)
				next[cols[i][j]] = cols[i][j + 1];
		}
