...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		auto realVec = VI{ vec[0] };
		if (vec.size() > 1)
			realVec.push_back(vec[1]);
		for (int i = 2; i < vec.size(); ++i) {
			if ((vec[i] != vec[i - 1]) || (vec[i] != vec[i - 2]))
				realVec.push_back(vec[i]);
		}

		std::vector<int> pairIndices;
		auto betterVec = VI{ realVec[0] };
		for (int i = 1; i < realVec.size(); ++i) {
			if (realVec[i] != realVec[i - 1])
				betterVec.push_back(realVec[i]);
			else
				pairIndices.push_back(betterVec.size() - 1);
		}
		auto greedyInRange = [&](const int begin, const int end, int& left, int& right, const int bad) {
			i64 res = 0;
			for (int i = begin; i < end; ++i) {
				if (betterVec[i] == left) {
					if (betterVec[i] == right)
						continue;
					right = betterVec[i];
					++res;
				}
				else if (betterVec[i] == right) {
					if (betterVec[i] == left)
						continue;
					left = betterVec[i];
					++res;
		...
		if (pairIndices.empty()) {
			std::cout << betterVec.size() << '\n';
			continue;
		}
		int left = 0, idx = 0;
		int lVal = -1, rVal = -1;
		i64 res = 0;
		while (left < betterVec.size()) {
			res += greedyInRange(left, pairIndices[idx], lVal, rVal, betterVec[pairIndices[idx]]);
			left = pairIndices[idx] + 1;
			++idx;
			if (idx >= pairIndices.size()) {
				res += greedyInRange(left, betterVec.size(), lVal, rVal, -1);
				break;
			}
		}
		std::cout << res << '\n';
