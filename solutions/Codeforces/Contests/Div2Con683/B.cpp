...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int n, m;
		std::cin >> n >> m;
		auto mat = readMatrixSz<int>(n, m);
		int noNegative = 0, smallestAbsVal = std::numeric_limits<int>::max(), totSum = 0;
		for (int i = 0; i < n; ++i) {
			for (int j = 0; j < m; ++j) {
				totSum += std::abs(mat[i][j]);
				smallestAbsVal = std::min(smallestAbsVal, std::abs(mat[i][j]));
				if (mat[i][j] < 0)
					++noNegative;
			}
		}
		if (noNegative % 2 == 0)
			...
			std::cout << totSum - 2*smallestAbsVal << '\n';
	}
}