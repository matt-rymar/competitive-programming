...
std::map<int, std::unordered_multiset<uint32_t>> getGroups(const std::unordered_multiset<uint32_t>& set)
{
	std::map<int, std::unordered_multiset<uint32_t>> res;
	for (auto i : set) {
		auto idx = msbIdx(i);
		if (i > 0)
			i -= (uint32_t(1) << idx);
...
int solve(const std::unordered_multiset<uint32_t>& set)
{
	if (set.size() <= 2)
		return 0;
	auto groups = getGroups(set);
	int above = groups.size() - 1;
	auto best = solve(groups.cbegin()->second);
	best += set.size() - groups.cbegin()->second.size() - above;
	--above;
	for (auto it = (++groups.cbegin()); it != groups.cend(); ++it) {
		auto cur = solve(it->second);
		cur += set.size() - it->second.size() - above - 1;
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int n;
		std::cin >> n;
		std::unordered_multiset<uint32_t> set;
		for (int i = 0; i < n; ++i) {
			int a;
			std::cin >> a;
			set.insert(a);
