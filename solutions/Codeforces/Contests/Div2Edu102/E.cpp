...
bool operator<(const NodeDist& nd1, const NodeDist& nd2)
{
	if (nd1.d != nd2.d)
		return nd1.d > nd2.d;
	if ((nd1.pathMax != nd2.pathMax) && (nd1.pathMax * nd2.pathMax < 0))
		return (nd2.pathMax < 0);
	if ((nd1.pathMin != nd2.pathMin) && (nd1.pathMin * nd2.pathMin < 0))
		return (nd1.pathMin < 0);
	return true;
}

std::vector<int64_t> dijkstra(const std::vector<std::vector<Neighbor>>& adj, int s)
{
	...
	for (auto neigh : adj[s]) {
		q.push(NodeDist{ neigh.id, neigh.w, -1, -1 });
		q.push(NodeDist{ neigh.id, 2*neigh.w, neigh.w, -1 });
		q.push(NodeDist{ neigh.id, 0, -1, neigh.w });
		q.push(NodeDist{ neigh.id, neigh.w, neigh.w, neigh.w });
		dists[neigh.id] = neigh.w;
	}
	while (!q.empty()) {
		auto nd = q.top();
		q.pop();
		...
		for (auto neigh : adj[nd.v]) {
			auto minWeight = std::min(nd.pathMin, neigh.w);
			auto maxWeight = std::max(nd.pathMax, neigh.w);
			auto newDelta = maxWeight - minWeight;
			auto newDist = nd.d + neigh.w + curDelta - newDelta;
			if ((dists[neigh.id] < 0) || (newDist < dists[neigh.id])) {
				dists[neigh.id] = newDist;
				q.push(NodeDist{ neigh.id, dists[neigh.id], minWeight, maxWeight });
			}
		}
	}
	return dists;
