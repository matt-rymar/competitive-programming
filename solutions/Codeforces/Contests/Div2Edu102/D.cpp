...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int n, m;
		std::cin >> n >> m;
		auto vec = readVecSz<char>(n);

		std::vector<P<int, int>> farPointsBefore(n + 1, std::make_pair(0, 0));
		std::vector<int> finalPos(n + 1, 0);
		for (int i = 0; i < n; ++i) {
			farPointsBefore[i + 1] = farPointsBefore[i];
			if (vec[i] == '+') {
				finalPos[i + 1] = finalPos[i] + 1;
				farPointsBefore[i + 1].second = std::max(farPointsBefore[i].second, finalPos[i + 1]);
			}
			else {
				finalPos[i + 1] = finalPos[i] - 1;
				farPointsBefore[i + 1].first = std::min(farPointsBefore[i].first, finalPos[i + 1]);
			}
		}

		std::vector<P<int, int>> farPointsAfter(n + 1, std::make_pair(0, 0));
		for (int i = n - 1; i >= 0; --i) {
			if (vec[i] == '+') {
				farPointsAfter[i].first = std::min(0, farPointsAfter[i + 1].first + 1);
				farPointsAfter[i].second = std::max(0, farPointsAfter[i + 1].second + 1);
			}
			else {
				farPointsAfter[i].first = std::min(0, farPointsAfter[i + 1].first - 1);
				farPointsAfter[i].second = std::max(0, farPointsAfter[i + 1].second - 1);
		...
		for (int i = 0; i < m; ++i) {
			...
			auto posAfter = 0;
			auto bestMin = 0, bestMax = 0;
			if (l > 0) {
				bestMin = farPointsBefore[l].first;
				bestMax = farPointsBefore[l].second;
				posAfter = finalPos[l];
			}
			if ((r+1) < n) {
				bestMin = std::min(bestMin, posAfter + farPointsAfter[r + 1].first);
				bestMax = std::max(bestMax, posAfter + farPointsAfter[r + 1].second);
			}

			std::cout << bestMax - bestMin + 1 << '\n';
		}
	}
}