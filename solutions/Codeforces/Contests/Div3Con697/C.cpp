...
constexpr i64 divisor = std::gcd(2020, 2021);
constexpr i64 certain = i64(2020) * 2021;
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		auto adjGirls = std::vector<std::unordered_set<int>>(a);
		auto adjBoys = std::vector<std::unordered_set<int>>(b);

		auto vec = readVecSz<i64>(k);
		auto vec2 = readVecSz<i64>(k);
		for (int i = 0; i < k; ++i) {
			adjGirls[vec[i] - 1].insert(vec2[i] - 1);
			adjBoys[vec2[i] - 1].insert(vec[i] - 1);
		}

		std::vector<i64> noAdjacent;
		for (int i = 0; i < k; ++i) {
			noAdjacent.push_back((adjGirls[vec[i] - 1].size() - 1)
				+ (adjBoys[vec2[i] - 1].size() - 1));
		}
		i64 res = 0;
		for (int i = 0; i < k; ++i)
			res += k - noAdjacent[i] - 1;
		std::cout << res / 2 << '\n';
	}
}