...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int n;
		std::cin >> n;
		auto a = readMatrixSz<char>(n, n);
		auto b = readMatrixSz<char>(n, n);

		auto testGiven = [&](bool x1) {
			auto xs = std::vector<bool>(n);
			auto ys = xs;
			xs[0] = x1;
			for (int i = 0; i < n; ++i)
				ys[i] = xs[0] ^ (a[i][0] != b[i][0]);
			for (int j = 1; j < n; ++j) {
				xs[j] = ys[0] ^ (a[0][j] != b[0][j]);
				for (int i = 1; i < n; ++i) {
					const auto req = xs[j] ^ (a[i][j] != b[i][j]);
					if (req != ys[i])
						return false;
				}
			}
		...
		if (testGiven(false) || testGiven(true))
			std::cout << "YES\n";
		else
			std::cout << "NO\n";
	}
