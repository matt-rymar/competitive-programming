...
std::vector<i64> suffSums(const std::vector<i64>& vec)
{
	auto res = vec;
	res.push_back(0);
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		auto weight = readVecSz<i64>(n);
		auto val = readVecSz<i64>(n);

		if (std::accumulate(weight.cbegin(), weight.cend(), i64(0)) < m) {
			std::cout << "-1\n";
			continue;
		}

		auto good = std::vector<i64>();
		auto bad = good;
		for (int i = 0; i < val.size(); ++i) {
			if (val[i] == 1)
				good.push_back(weight[i]);
			else
				bad.push_back(weight[i]);
		}
		std::sort(good.begin(), good.end());
		std::sort(bad.begin(), bad.end());

		auto goodSums = suffSums(good);
		auto badSums = suffSums(bad);
		i64 i = good.size(), j = bad.size();
		while ((i >= 0) && (goodSums[i] < m))
			--i;
		i = std::max(i, i64(0));
		while ((goodSums[i] + badSums[j]) < m)
			--j;
		i64 best = (good.size() - i) + (bad.size() - j) * 2;
		while ((++i) <= good.size()) {
			while ((j >= 0) && ((goodSums[i] + badSums[j]) < m))
				--j;
			if (j < 0)
				break;
			best = std::min(best, i64(good.size() - i) + i64(bad.size() - j) * 2);
		}
		std::cout << best << '\n';
	}
}