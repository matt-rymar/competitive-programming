...
// Computes (n choose k) % mod, given a vector of modular inverses (for example, the one computed by modInverseRange above)
template<typename T>
...
const auto modInvs = modInverseRange<i64>(mod, 1005);
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		while (k > 0) {
			lastcount = 0;
			auto val = vec.back();
			while ((!vec.empty()) && (vec.back() == val)) {
				++lastcount;
				vec.pop_back();
			}
			k -= lastcount;
		}
		std::cout << nChooseKMod<i64>(lastcount, k + lastcount, mod, modInvs) << '\n';
	}
}