...
int64_t val(const int64_t n) { return (1LL << (n + 1)) - 2; }

int64_t search(const int64_t k, int64_t hi = 61)
{
	int64_t lo = 1;
	while (lo < hi) {
		auto mid = (hi + lo) / 2;
...
std::pair<int, std::vector<int>> solve(int64_t k)
{
	std::vector<int> res;
	int tot = 0;
	while (k) {
