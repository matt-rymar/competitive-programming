...
int main()
{
	...
	std::cin >> t;
	for (int i = 0; i < t; ++i) {
		auto vec = readVec<int64_t>();
		int64_t sum = 0;
		for (int i = 1; i < vec.size(); ++i) {
			auto diff = std::abs(vec[i] - vec[i - 1]);
			sum += diff;
		}
		int64_t best = std::min(sum - std::abs(vec[0] - vec[1]), sum - std::abs(vec.back() - vec[vec.size() - 2]));
		for (int i = 1; (i+1) < vec.size(); ++i) {
			auto left = std::abs(vec[i] - vec[i - 1]) + std::abs(vec[i] - vec[i + 1]) - std::abs(vec[i - 1] - vec[i + 1]);
			best = std::min(best, sum - left);
		}
		std::cout << best << '\n';
	}
}