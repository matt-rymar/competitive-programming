...
void dfs_rec(const VVI& adj, VI& parent, VI& depth, VI& minDepth, int cur)
{
	for (auto i : adj[cur]) {
		if (i == parent[cur])
			continue;
		parent[i] = cur;
		dfs_rec(adj, parent, depth, minDepth, i);
		depth[cur] = std::max(depth[cur], 1 + depth[i]);
		minDepth[cur] = std::min(minDepth[cur], 1 + minDepth[i]);
	}
	if (minDepth[cur] == POS_INF<int>)
		minDepth[cur] = 0;
}

std::tuple<std::vector<int>, std::vector<int>, std::vector<int>> dfs(const VVI& adj)
{
	auto parent = std::vector<int>(adj.size());
	auto depth = parent;
	auto minDepth = VI(adj.size(), POS_INF<int>);
	dfs_rec(adj, parent, depth, minDepth, 0);
	return { parent, depth, minDepth };
}

int minPossible_rec(const VVI& adj, const VI& parent, const VI& depth, const VI& minDepth, int cur)
{
	if ((adj[cur].size() == 1) && (adj[cur].front() == parent[cur]))
		return 0;
	auto minForSubtrees = NEG_INF<int>;
	auto minForTransitions = NEG_INF<int>;
	for (auto i : adj[cur]) {
		if (i == parent[cur])
			continue;
		auto minSubtree = minPossible_rec(adj, parent, depth, minDepth, i);
		minForSubtrees = std::max(minForSubtrees, minSubtree);
		minForTransitions = std::max(minForTransitions, minDepth[i]);
	}
	if (adj[cur].size() == 2)
		return std::max(minForSubtrees, minForTransitions + 1);
	return std::max(minForSubtrees, minForTransitions + 2);
}

int minPossible(const VVI& adj, const VI& parent, const VI& depth, const VI& minDepth)
{
	auto minForSubtrees = NEG_INF<int>;
	std::deque<int> q;
	for (auto i : adj[0]) {
		const auto minSubtree = minPossible_rec(adj, parent, depth, minDepth, i);
		minForSubtrees = std::max(minForSubtrees, minSubtree);
		const auto minD = minDepth[i];
		if (q.empty())
			q.push_back(minD);
		else if (q.size() == 1) {
			if (minD > q.front())
	...
	auto minForTransitions = q.front() + 1;
	if (q.size() > 1)
		minForTransitions = std::max(minForTransitions, q.back() + 2);
	return std::max(minForSubtrees, minForTransitions);
}
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int n;
		std::cin >> n;
		auto adj = readGraphUn(n, n - 1);
		auto [parent, depth, minDepth] = dfs(adj);
		std::cout << minPossible(adj, parent, depth, minDepth) << '\n';
	}
}