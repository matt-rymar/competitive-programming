...
std::vector<int> nextSmaller(const std::vector<T>& vec)
{
	...
	for (int i = (int)vec.size() - 2; i >= 0; --i) {
		auto j = i + 1;
		while ((j < vec.size()) && (vec[i] <= vec[j])) // Change to vec[i] > vec[j] to get next larger, vec[i] <= vec[j] to get next strictly smaller, etc.
			j = res[j];
		res[i] = j;
	}
	return res;
...
std::vector<int> prevSmaller(const std::vector<T>& vec)
{
	...
	for (int i = 1; i < vec.size(); ++i) {
		auto j = i - 1;
		while ((j >= 0) && (vec[i] <= vec[j])) // Change to vec[i] > vec[j] to get next larger, vec[i] <= vec[j] to get next strictly smaller, etc.
			j = res[j];
		res[i] = j;
	}
	return res;
