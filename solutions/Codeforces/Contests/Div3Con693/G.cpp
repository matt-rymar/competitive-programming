...
std::vector<int> getDists(const VVI& adj, int s)
{
	auto res = VI(adj.size(), -1);
	res[s] = 0;
	std::queue<int> q;
	q.push(s);
	while (!q.empty()) {
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int n, m;
		std::cin >> n >> m;
		auto adj = readGraphDir(n, m);

		auto dists = getDists(adj, 0);
		auto vals = VI(n);
		auto visited = std::vector<bool>(n, false);
		y_combinator([&](auto dfs, int s) -> int {
			if (visited[s])
				return vals[s];
			vals[s] = dists[s];
			for (auto i : adj[s]) {
				if (dists[i] <= dists[s])
					vals[s] = std::min(vals[s], dists[i]);
				else
					vals[s] = std::min(vals[s], dfs(i));
			}
			visited[s] = true;
			return vals[s];
		})(0);
