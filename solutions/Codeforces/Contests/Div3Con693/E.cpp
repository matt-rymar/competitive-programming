...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		for (int i = 0; i < n; ++i) {
			int x, y;
			std::cin >> x >> y;
			vec.emplace_back(std::min(x, y), std::max(x, y));
		}

		auto cpy = sortedTracedCopy(vec);
		
		auto best = cpy.front().second.second;
		auto bestIdx = cpy.front().first;
		auto available = POS_INF<int>;
		auto availableIdx = -1;
		auto res = VI(n, -2);
		for (int i = 1; i < n; ++i) {
			if (cpy[i - 1].second.first < cpy[i].second.first) {
				if (best < available) {
					available = best;
					availableIdx = bestIdx;
				}
			}
			if (available < cpy[i].second.second)
				res[cpy[i].first] = availableIdx;
			if (cpy[i].second.second < best) {
				best = cpy[i].second.second;
				bestIdx = cpy[i].first;
			}
		}
