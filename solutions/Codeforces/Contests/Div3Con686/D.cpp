...
std::vector<int> primeSieve(int n)
{
	...
	for (int i = 3; i <= n; i += 2) { // Technically 2*i <= n would be enough to find all primes, but since we need to also add primes > n/2 into the vector, might as well do it in this loop
		if (!isPrime[i])
			continue;
		res.push_back(i);
...
// Returns a list of prime factors of n (which are contained in "primes"), alongside with the power the factor is there with
std::vector<std::pair<int64_t, int>> primeFactorization(const std::vector<int>& primes, int64_t n)
{
...
int main()
{
	...
	const auto primes = primeSieve(100'010);
	...
	for (int i = 0; i < t; ++i) {
		int64_t n;
		std::cin >> n;
		auto facts = primeFactorization(primes, n);
		std::sort(facts.begin(), facts.end(), [](const auto& p1, const auto& p2) {return p1.second > p2.second; });
		std::cout << facts.front().second << '\n';
		uint64_t cur = 1;
		int curFact = 0;
		for (int i = 0; i < facts.front().second; ++i) {
			auto remLen = facts.front().second - i;
			while ((curFact < facts.size()) && (facts[curFact].second == remLen)) {
				cur *= facts[curFact].first;
				++curFact;
			}
			std::cout << cur << ' ';
		}
