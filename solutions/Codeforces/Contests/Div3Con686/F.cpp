...
std::vector<int> getPrefixMaxes(const std::vector<int>& vec)
{
	std::vector<int> res{ vec[0] };
	for (int i = 1; i < vec.size(); ++i)
		res.push_back(std::max(res.back(), vec[i]));
	return res;
}

auto findMids(const std::vector<int>& vec, const RMQ<int>& rmq, const int idx, const int curMax)
{
	int lo = 0, hi = idx - 1;
	while (lo < hi) {
		auto mid = lo + (hi - lo) / 2;
		if (rmq.getMin(mid, idx - 1) >= curMax)
			hi = mid;
		else
			lo = mid + 1;
	}
	...
	if (rmq.getMin(lo, idx - 1) == curMax)
		++lo;
	return std::make_pair(lower, lo - 1);
}

std::optional<std::pair<int, int>> solve(const std::vector<int>& vec)
{
	auto maxes = getPrefixMaxes(vec);
	auto rmq = RMQ(vec);
	auto helper = vec;
	std::iota(helper.begin(), helper.end(), 0);
	auto curMax = vec.back();
	for (int i = vec.size() - 1; i >= 2; --i) { // i >= 2 since middle and prefix need to be at least one in size
		curMax = std::max(curMax, vec[i]);
		auto [prefFrom, prefTo] = std::equal_range(maxes.cbegin(), maxes.cbegin() + i, curMax);
		if (prefFrom == prefTo)
			continue;
		auto prefLo = (int)std::distance(maxes.cbegin(), prefFrom);
		auto prefHi = (int)std::distance(maxes.cbegin(), prefTo) - 1;
		auto [midLo, midHi] = findMids(vec, rmq, i, curMax);
		if (midLo > midHi)
			continue;
		//std::cout << prefLo << ' ' << prefHi << ' ' << midLo << ' ' << midHi << '\n';
		if ((prefHi + 1 < midLo) || ((prefLo == midLo) && (midLo == midHi)))
			continue;
		return { {std::max(prefLo + 1, midLo), i - 1} };
	}
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		if (auto opt = solve(vec); opt)
			std::cout << "YES\n" << opt->first << ' ' << opt->second - opt->first + 1<< ' ' << vec.size() - opt->second - 1 << '\n';
		else
			std::cout << "NO\n";
	}
}