...
bool possible(const V<i64>& a, const V<i64>& b)
{
	return std::accumulate(a.cbegin(), a.cend(), i64(0))
		== std::accumulate(b.cbegin(), b.cend(), i64(0));
}
...
int main()
{
	...
	//std::cin >> t;
	for (int i = 0; i < t; ++i) {
		auto a = readVec<int64_t>();
		auto b = readVec<int64_t>();
		if (!possible(a, b)) {
			std::cout << -1 << '\n';
			continue;
		}
		...
		for (int i = 0, j = 0; (i < a.size()) || (j < b.size()); ) {
			// No need to be worried about out-of-bounds since we know the instance is solvable
			if (curSumA == 0) {
				curSumA = a[i++];
				curSumB = b[j++];
			}
			while (curSumA < curSumB)
				curSumA += a[i++];
			while (curSumB < curSumA)
				curSumB += b[j++];
			if (curSumA == curSumB) {
				++len;
				curSumA = curSumB = 0;
