...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		auto vec = readVec<std::pair<i64, int>>();

		std::priority_queue<i64, std::vector<i64>, std::greater<i64>> newAs, newBs;
		std::priority_queue<i64> oldAs, oldBs;
		for (auto [i, t] : vec) {
			if (t == 1)
				newAs.push(i);
			else
		...
		int i = 0;
		for (; i < vec.size(); ++i) {
			if ((newBs.empty()) || ((!newAs.empty()) && (newAs.top() <= newBs.top()))) {
				if ((2 * int(oldBs.size())) > (int(oldAs.size()) - 2)) {
					curSum += newAs.top();
					oldAs.push(newAs.top());
					newAs.pop();
				}
				else if (newBs.empty()) {
					break;
					...
					curSum += newBs.top();
					oldBs.push(newBs.top());
					newBs.pop();
				}
			}
			else {
				if ((2 * int(oldAs.size())) > (int(oldBs.size()) - 2)) {
					curSum += newBs.top();
					oldBs.push(newBs.top());
					newBs.pop();
				}
