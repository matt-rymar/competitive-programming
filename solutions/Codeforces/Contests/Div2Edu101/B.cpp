...
std::vector<int64_t> prefixSum(const V<i64>& vec)
{
	V<i64> res{ 0 };
	for (auto i : vec)
...
std::vector<int> nextNeg(const V<i64>& vec)
{
	auto res = V<int>(vec.size(), -1);
...
int main()
{
	...
	std::cin >> t;
	for (int k = 0; k < t; ++k) {
		auto as = readVec<int64_t>();
		auto bs = readVec<i64>();
		/*auto apref = prefixSum(as);
		auto aneg = nextNeg(as);
		auto bpref = prefixSum(bs);
		auto bneg = nextNeg(bs);
		int64_t res = 0;
		int i = 0, j = 0;
		int64_t sum = 0;
		...
		std::cout << maxPref(as) + maxPref(bs) << '\n';
	}
}