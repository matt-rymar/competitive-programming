...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		i64 minHeight = std::max(i64(0), vec[0] - (k-1)), maxHeight = vec[0] + k - 1;
		bool possible = true;
		for (int i = 1; i < (vec.size() - 1); ++i) {
			const auto h = vec[i];
			if (((h + k-1) < minHeight) || (h > maxHeight)) {
				possible = false;
				break;
			}
			minHeight = std::max(i64(0), std::max(minHeight, h) - (k - 1));
			maxHeight = std::min(maxHeight, h + k-1) + (k - 1);
		}
		if (possible && (minHeight <= vec.back()) && (vec.back() <= maxHeight))
			std::cout << "YES\n";
		else
			std::cout << "NO\n";
	}
