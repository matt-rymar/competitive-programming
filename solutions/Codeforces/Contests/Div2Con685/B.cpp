	...
	int main()
	{
		...
		for (int i = 0; i < t; ++i) {
			int n, q;
			std::cin >> n >> q;
			auto vec = readVectorSz<char>(n);
			auto isOneLeft = std::vector<bool>(n, false);
			auto isZeroLeft = isOneLeft;
			for (int i = 1; i < n; ++i) {
				isOneLeft[i] = isOneLeft[i - 1] || (vec[i - 1] == '1');
				isZeroLeft[i] = isZeroLeft[i - 1] || (vec[i - 1] == '0');
			}
			auto isOneRight = std::vector<bool>(n, false);
			auto isZeroRight = isOneRight;
			for (int i = n - 2; i >= 0; --i) {
				isOneRight[i] = isOneRight[i + 1] || (vec[i + 1] == '1');
				isZeroRight[i] = isZeroRight[i + 1] || (vec[i + 1] == '0');
			}
			for (int i = 0; i < q; ++i) {
				int l, r;
				std::cin >> l >> r;
				...
				bool res = false;
				if (vec[l] == '0')
					res = res || isZeroLeft[l];
				else
					res = res || isOneLeft[l];
				if (vec[r] == '0')
					res = res || isZeroRight[r];
				else
					res = res || isOneRight[r];
				if (res)
					std::cout << "YES\n";
				else
					std::cout << "NO\n";
