...
std::pair<int64_t, std::set<int>> solve(const int n, const int m, const std::vector<std::pair<int, std::vector<int>>>& columns)
{
	

	//auto dim = std::accumulate(coverNumbers.cbegin(), coverNumbers.cend(), i64(0)) / 2;
	//return { modExp(i64(2), dim, MOD), taken };
}
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		int n, m;
		std::cin >> n >> m;
		auto columns = std::vector<std::pair<int, std::vector<int>>>();
		for (int i = 0; i < n; ++i) {
			int k;
			std::cin >> k;
		...
		for (int i = 0; i < columns.size(); ++i) {
			auto col = (columns[i].second.size() == 1) ? 
				std::make_pair(-1, columns[i].second[0]) : 
				std::make_pair(columns[i].second[0], columns[i].second[1]);
			if (seen.count(col) > 0)
				toRemove.insert(i);
			else
				seen.insert(col);
		...
		columns.erase(std::remove_if(columns.begin(), columns.end(), 
			[&](const auto& p) {return toRemove.count(p.first) > 0; }), columns.end());

		// Compute an optimal solution
		auto [sz, goodCols] = solve(n, m, columns);
		std::cout << sz << ' ' << goodCols.size() << '\n';
		for (auto i : goodCols)
			std::cout << i + 1 << ' ';
		std::cout << '\n';
	}
