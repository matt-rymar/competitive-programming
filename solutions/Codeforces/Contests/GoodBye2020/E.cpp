...
constexpr uint64_t lastPower = 60;
...
int main()
{
	...
	std::cin >> t;
	for (int i = 0; i < t; ++i) {
		auto vec = readVec<uint64_t>();
		const auto n = vec.size();
		auto onesCount = std::vector<uint64_t>(60, 0);
		for (auto i : vec) {
			for (uint64_t j = 0; j < lastPower; ++j)
				onesCount[j] += (i & (uint64_t(1) << j)) > 0;
		}

		auto placeVals = std::vector<uint64_t>(60, 1);
		for (int i = 1; i < placeVals.size(); ++i)
			placeVals[i] = (placeVals[i - 1] * 2) % MOD;
		...
		auto sums = [&](int idx) {
			...
			for (uint64_t j = 0; j < lastPower; ++j) {
				...
				if ((num & mask) > 0) {
					orSum = (orSum + n * placeVals[j]) % MOD;
					andSum = (andSum + onesCount[j] * placeVals[j]) % MOD;
				}
				else {
					orSum = (orSum + onesCount[j] * placeVals[j]) % MOD;
				}
			}
			return std::make_pair(orSum, andSum);
		};

		uint64_t res = 0;
		for (int i = 0; i < n; ++i) {
			auto [orSum, andSum] = sums(i);
			res = (res + ((orSum * andSum) % MOD)) % MOD;
		}
		std::cout << res << '\n';
	}
}