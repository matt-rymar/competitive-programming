...
std::optional<std::vector<int>> spanningColor(const VVI& adj)
{
	...
	y_combinator([&](auto dfs, int s) -> void {
		if (visited[s])
			return;
		visited[s] = true;
		if (colors[s] == Color::none) {
			colors[s] = Color::red;
			for (auto j : adj[s])
				colors[j] = Color::blue;
		}
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int n, m;
		std::cin >> n >> m;
		auto adj = readGraphUn(n, m);

		if (auto opt = spanningColor(adj); opt) {
			std::cout << "YES\n" << opt->size() << '\n';
			for (auto i : *opt)
				std::cout << i + 1 << ' ';
			std::cout << '\n';
