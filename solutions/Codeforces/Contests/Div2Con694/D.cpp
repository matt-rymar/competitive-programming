...
std::vector<int> primeSieve(const int n)
{
	...
	for (int64_t i = 3; i <= n; i += 2) { // Technically i*i <= n would be enough to find all primes, but since we need to also add primes > sqrt(n) into the vector, might as well do it in this loop
		if (!isPrime[i])
			continue;
		res.push_back(i);
...
const auto primes = primeSieve(1'005);
...
int main()
{
	...
	std::cin >> t;
	for (int i = 0; i < t; ++i) {
		auto vec = readVec<int>([](auto x) {return reduce(x); });
		...
		for (auto [num, count] : counts) {
			if (count & 1)
				bestOdd = std::max(bestOdd, count);
			else if (num > 1)
				totalEven += count;
			bestOverall = std::max(bestOverall, count);
		}
		auto bestAfter = std::max(totalEven + counts[1], bestOdd);
