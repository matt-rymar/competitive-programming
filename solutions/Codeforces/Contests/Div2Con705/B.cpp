...
char nextMirror(char c, const int max)
{
	if (c <= '2')
		return c;
	if ((c == '5') || (c == '8'))
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		/*if (s[0] != nextMirror(s[0], h)) {
			const auto next = nextMirror(s[0], h);
			if (next == 0) {
				s[0] = s[1] = s[3] = s[4] = '0';
				std::cout << s << '\n';
				continue;
		...
		const auto nextHour = (s[0] == ('0' + (h - 1))) ? (h % 10) : 9;
		if (s[1] != nextMirror(s[1], nextHour)) {
			const auto next = nextMirror(s[1], nextHour);
			if (next == 0) {
				s[1] = s[3] = s[4] = '0';
				std::cout << s << '\n';
				continue;
		...
		if (s[3] != nextMirror(s[3], m)) {
			const auto next = nextMirror(s[3], m);
			if (next == 0) {
				s[3] = s[4] = '0';
				std::cout << s << '\n';
				continue;
		...
		const auto nextMinute = (s[3] == ('0' + (m - 1))) ? (m % 10) : 9;
		if (s[4] != nextMirror(s[4], nextMinute)) {
			const auto next = nextMirror(s[4], nextMinute);
			s[4] = next;
		}
		std::cout << s << '\n';*/


		const auto maxM = m / 10 + ((m % 10) != 0);
		const auto maxH = h / 10 + ((h % 10) != 0);
		const auto nextMinute = (s[3] == ('0' + maxM - 1)) ? (m % 10) : 9;
		const auto nextHour = (s[0] == ('0' + maxH - 1)) ? (h % 10) : 9;
		if (s[4] != nextMirror(s[4], std::min(nextMinute, maxH))) {
			const auto next = nextMirror(s[4], std::min(nextMinute, maxH));
			if (next == '0') {
				s[3] = '0';
				s[1]++;
				if (s[1] - '0' >= nextHour) {
		...
		const auto upperM = std::min(maxM, (s[4] - '0' == maxH - 1) ? (h % 10) : 10);
		if (s[3] != nextMirror(s[3], upperM)) {
			const auto next = nextMirror(s[3], upperM);
			if (next == '0') {
				s[3] = s[4] = '0';
				s[1]++;
				if (s[1] - '0' >= nextHour) {
		...
		if (s[1] != nextMirror(s[1], std::min(nextHour, )) {
			const auto next = nextMirror(s[1], nextHour);
			if (next == '0') {
				s[1] = s[3] = s[4] = '0';
				if (s[0] - '0' == maxM)
		...
		if (s[0] != nextMirror(s[0], maxH)) {
			const auto next = nextMirror(s[0], maxH);
			if (next == '0')
				s[0] = s[1] = s[3] = s[4] = '0';
			s[0] = next;
		}
