...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		auto tree = DefSegmentTree([&](auto i) {return Data{ .gcd = vec[i], .rangeLen = 1 }; }, vec.size());
		for (int i = 0; i < q; ++i) {
			int idx, x;
			std::cin >> idx >> x;
			--idx;
			tree.pointUpdate(idx, UpdateData{ .val = x });
			std::cout << (tree.query(0, vec.size() - 1).gcd % MOD) << '\n';
		}
	}
}