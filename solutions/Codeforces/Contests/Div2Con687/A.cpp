...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int64_t n, m, r, c;
		std::cin >> n >> m >> r >> c;
		auto upperLeft = r - 1 + c - 1;
		auto upperRight = m - c + r - 1;
		auto lowerLeft = n - r + c - 1;
		auto lowerRight = n - r + m - c;
		std::cout << std::max(std::max(std::max(lowerLeft, lowerRight), upperRight), upperLeft) << '\n';
	}
}