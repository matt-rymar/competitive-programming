...
int64_t colorCost(const std::vector<int>& vec, const int k, const int c)
{
	int64_t res = 0;
	for (int l = 0; l < vec.size(); ) {
		while ((l < vec.size()) && (vec[l] == c))
			++l;
		if (l < vec.size()) {
			res += 1;
			l += k;
...
std::vector<int64_t> needToPass(std::string_view sv, int64_t k)
{
	std::vector<int64_t> res(sv.size(), -1);
	for (int i = sv.size() - 1; i >= std::max((int64_t)0, (int64_t)sv.size() - k); --i)
		res[i] = (sv[i] == '0') ? 1 : 0;
	for (int i = sv.size() - k - 1; i >= 0; --i) {
		res[i] = res[i + k];
		if (sv[i] == '0')
			++res[i];
	}
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		int64_t x, y;
		std::cin >> x >> y;
		auto passes = needToPass(s, k);
		int64_t res = passes[p] * x;
		for (int j = p + 1; (j < n) && ((n - (j-p)) >= p); ++j)
			res = std::min(res, ((j - p) * y) + (passes[j] * x));
		std::cout << res << '\n';
	}
}