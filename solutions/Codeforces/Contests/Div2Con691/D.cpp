...
int main()
{
	...
	//std::cin >> t;
	for (int i = 0; i < t; ++i) {
		auto vec = readVec<std::pair<int, int>>();
		auto dp = std::vector<std::vector<std::pair<double, double>>>(vec.size() + 1, std::vector<std::pair<double, double>>(vec.size() + 1, std::make_pair(0.0, 0.0)));
		for (int i = 1; i <= vec.size(); ++i) {
			for (int k = 1; k <= vec.size(); ++k) {
				dp[i][k] = dp[i - 1][k];
				auto waterUsed = std::min(2.0 * (vec[i - 1].second - vec[i - 1].first), std::max(0.0, dp[i - 1][k - 1].first));
				if ((dp[i - 1][k - 1].second + vec[i - 1].first + waterUsed) > dp[i][k].second)
					dp[i][k] = std::make_pair(dp[i - 1][k - 1].first - waterUsed, (dp[i - 1][k - 1].second + vec[i - 1].first + waterUsed));
				waterUsed = std::min(2.0 * (-1.0*std::min(dp[i - 1][k].first, 0.0)), 1.0*vec[i - 1].first);
				if (dp[i - 1][k].second + waterUsed > dp[i][k].second)
					dp[i][k] = std::make_pair(dp[i - 1][k].first + waterUsed, dp[i - 1][k].second + waterUsed);
			}
		}
		for (int i = 1; i <= vec.size(); ++i) {
			std::cout << dp[vec.size()][i].second << ' ';
		}
		std::cout << '\n';
	}
}