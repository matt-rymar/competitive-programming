...
bool canDo(int x, int y, int dist)
{
	static auto memo = std::unordered_map<int, std::unordered_map<int, std::unordered_map<int, std::unordered_set<int>>>>();
	memo[0][0][0].insert(0); memo[0][0][0].insert(1);
	if (memo[x][y].count(0))
		return true;
	if (canDo(x-1, y, !dir, dist-1))
}
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		const int offset = n;
		const int sz = 3*n;
		auto vis = std::vector<std::vector<std::vector<int>>>(sz, std::vector<V<int>>(sz, V<int>(sz, -1)));
		auto flags = std::vector<std::vector<std::vector<bool>>>(sz, std::vector<V<bool>>(sz, V<bool>(sz, false)));
		vis[offset][offset][0] = vis[offset][offset][1] = n;
		std::priority_queue<std::pair<int, std::pair<int, std::pair<int, int>>>> q;
		q.emplace(n, std::make_pair(0, std::make_pair(offset, offset)));
		q.emplace(n, std::make_pair(1, std::make_pair(offset, offset)));
		int res = (n % 4 == 0);
		auto counted = std::vector<std::vector<bool>>(sz, V<bool>(sz, false));
		counted[offset][offset] = res;
		while (!q.empty()) {
			auto [dist, lol] = q.top();
			auto [dir, p] = lol;
			q.pop();
			if (flags[p.first][p.second][dir])
				continue;
			flags[p.first][p.second][dir] = true;
			if (dir == 0) {
				if (!counted[p.first - 1][p.second] && ((dist - 1) % 4 == 0)) {
					++res;
					counted[p.first - 1][p.second] = true;
				}
				if ((dist > 1) && vis[p.first - 1][p.second][!dir] < dist - 1) {
					q.emplace(dist - 1, std::make_pair(!dir, std::make_pair(p.first - 1, p.second)));
					vis[p.first - 1][p.second][1] = dist - 1;
				}
				if (!counted[p.first + 1][p.second] && ((dist - 1) % 4 == 0)) {
					++res;
					counted[p.first + 1][p.second] = true;
				...
				if (!counted[p.first][p.second - 1] && ((dist - 1) % 4 == 0)) {
					++res;
					counted[p.first][p.second - 1] = true;
				}
				if ((dist > 1) && vis[p.first][p.second - 1][!dir] < dist - 1) {
					q.emplace(dist - 1, std::make_pair(!dir, std::make_pair(p.first, p.second - 1)));
					vis[p.first][p.second - 1][1] = dist - 1;
				}
				if (!counted[p.first][p.second - 1] && ((dist - 1) % 4 == 0)) {
					++res;
					counted[p.first][p.second - 1] = true;
