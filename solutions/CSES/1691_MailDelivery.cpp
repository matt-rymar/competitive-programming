...
// Checks whether the input undirected graph is connected
bool isConnected(const std::vector<std::vector<int>>& adj)
{
	...
	return std::accumulate(visited.cbegin(), visited.cend(), true, std::logical_and{});
}

// Returns true if the given connected undirected graph has an Euler cycle
bool isEulerianUndirected(const std::vector<std::vector<int>>& adj)
{
	const auto odds = std::count_if(adj.cbegin(), adj.cend(), [](const auto& v) { return (v.size() % 2) == 1; });
	return (odds == 0) && isConnected(adj);
}
...
// For an example use, see Leetcode 753. Cracking the Safe https://leetcode.com/problems/cracking-the-safe/
...
int main()
{
	...
	for (int _testCase = 0; _testCase < t; ++_testCase) {
		...
		const auto cycle = eulerCycle(adj, false);
		if (cycle.size() != (m + 1))
			std::cout << "IMPOSSIBLE\n";
		else {
			for (auto i : cycle)
