...
// WARNING: this is wholly untested; it may not even compile. Only test-ish thing is this task: https://leetcode.com/problems/minimum-number-of-days-to-disconnect-island/submissions/
std::unordered_set<int> getCutVertices(const std::vector<std::vector<int>>& adj)
{
	...
	auto dfs = y_combinator([&](auto dfs, int cur, int parent) -> void {
		...
		for (auto i : adj[cur]) {
			...
			if ((low[i] >= arrival[cur]) && ((arrival[cur] != 0) || (rootComponents > 1)))
				res.insert(cur);
		}
		});
...
int main()
{
	...
	while (true) {
		...
		if ((n == 0) && (m == 0))
			break;
		const auto adj = readGraph(n, m, false);
		const auto cvs = getCutVertices(adj);
		std::cout << cvs.size() << '\n';
	}
}