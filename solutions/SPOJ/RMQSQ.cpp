...
template<typename T>
int logCeil(T n) // Ceiling of binary log of n; log(0) = -1 here
{
	if (n <= 0)
		return -1;
	int res = -!(n & (n-1)); // for powers of two, the result needs to be decremented by one
	while (n) {
		++res;
		n >>= 1;
	}
...
// See https://www.spoj.com/problems/RMQSQ/
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		auto rmq = RMQ<int>(vec);
		int q;
		std::cin >> q;
		for (int j = 0; j < q; ++j) {
			int l, r;
			std::cin >> l >> r;
			std::cout << rmq.getMin(l, r) << '\n';
		}
	}
}