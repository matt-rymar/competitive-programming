...
std::vector<int> nextSmaller(const std::vector<T>& vec)
{
	...
	for (int i = (int)vec.size() - 2; i >= 0; --i) {
		auto j = i + 1;
		for (; (j < vec.size()) && (vec[i] <= vec[j]); j = res[j]) // Change to vec[i] > vec[j] to get next (non-strict) larger, vec[i] <= vec[j] to get next strictly smaller, etc.
			;
		res[i] = j;
	}
	return res;
...
std::vector<int> prevSmaller(const std::vector<T>& vec)
{
	...
	for (int i = 1; i < vec.size(); ++i) {
		auto j = i - 1;
		for (; (j >= 0) && (vec[i] <= vec[j]); j = res[j]) // Change to vec[i] > vec[j] to get next (non-strict) larger, vec[i] <= vec[j] to get next strictly smaller, etc.
			;
		res[i] = j;
	}
	return res;
...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		...
		auto solve = y_combinator([&](auto solve, auto i) -> i64 {
			if (i == 0)
				return 1;
			const auto smaller = solve(i - 1);
			if (!ops[i - 1])
				return smaller;
			return smaller + ((i64(1)) << i);
			});
		std::cout << solve(n) << '\n';
	}
}