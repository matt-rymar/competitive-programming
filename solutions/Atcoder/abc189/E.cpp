...
std::array<std::array<i64, 2>, 2> mult(const std::array<std::array<i64, 2>, 2>& mat1, const std::array<std::array<i64, 2>, 2>& mat2)
{
	std::array<std::array<i64, 2>, 2> res;
...
int main()
{
	...
	//std::cin >> t;
	for (int i = 0; i < t; ++i) {
		auto vec = readVec<std::pair<i64, i64>>();
		int M;
		std::cin >> M;
		std::vector<std::vector<i64>> perm{ {0, 1} };
		std::vector<std::vector<i64>> mult{ {1, 1} };
		std::vector<std::vector<i64>> offset{ {0, 0} };
		for (int i = 0; i < M; ++i) {
			int op;
			std::cin >> op;
			if (op <= 2) {
				if (op == 1) {
					perm.push_back({ !perm.back()[0], !perm.back()[1] });
					mult.push_back({ mult.back()[perm.back()[0]], -mult.back()[perm.back()[1]] });
					offset.push_back({ offset.back()[perm.back()[0]], offset.back()[perm.back()[1]] });
				}
				else {
					perm.push_back({ !perm.back()[0], !perm.back()[1] });
					mult.push_back({ -mult.back()[perm.back()[0]], mult.back()[perm.back()[1]] });
