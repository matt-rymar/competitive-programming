...
int main()
{
	...
	for (int i = 0; i < t; ++i) {
		int n;
		std::cin >> n;
		auto vec = readVecSz<i64>(1 << n);

		auto first = std::max_element(vec.cbegin(), vec.cbegin() + (1 << (n - 1)));
		auto sec = std::max_element(vec.cbegin() + (1 << (n - 1)), vec.cend());
		if ((*first) < (*sec))
			std::cout << std::distance(vec.cbegin(), first) + 1 << '\n';
		else
			std::cout << std::distance(vec.cbegin(), sec) + 1 << '\n';
	}
}